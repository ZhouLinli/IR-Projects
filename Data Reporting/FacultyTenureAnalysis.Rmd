...
#order based on non tenure
dep.order<-data%>%group_by(Department,`Tenure Status`)%>%
  summarise(cnt=n())%>%mutate(prt=percent(cnt/sum(cnt),digits = 0))%>%
  filter(`Tenure Status`=="Non Tenure")%>%arrange(-prt)%>%
  mutate(dep.f=factor(Department))
```
...
data%>%filter(Department %in% ndep[ndep$cnt>20,1]$Department ) %>%
  ...
  scale_x_discrete(limits = dep.order$dep.f[dep.order$dep.f %in% ndep[ndep$cnt>20,1]$Department])+
...
data%>%filter(Department %in% ndep[ndep$cnt%in%seq(5,20,by=1),1]$Department ) %>%
  ...
  summarise(cnt=n_distinct(ID))%>%mutate(prt=percent(cnt/sum(cnt),digits = 0))%>%
  ggplot(aes(x=Department,y=prt,fill=`Tenure Status`))+geom_bar(stat = "identity")+
  scale_x_discrete(limits = dep.order$dep.f[dep.order$dep.f %in% 
