...
#approach 1
#data merge full
fis.s<-fis.s%>%mutate(RespondentOverall="Respondent")
allstu22<-allstu22%>%mutate(RespondentOverall="Overall")%>%rename(ppid=`People Code Id`)
fulldf<-full_join(fis.s,allstu22)

#response rate
num.response=fulldf%>%filter(RespondentOverall=="Respondent")%>%nrow()
num.overall=fulldf%>%filter(RespondentOverall=="Overall")%>%nrow()
...
allstu22.s<-allstu22.s%>%mutate(ResAll=
                        ...
                        "Respond","Not Respond"))
allstu22.s%>%count(ResAll) #314 person, 195 responded, 177 have ppid, 18 no ppid
```



```{r making fig for respondents}
#check na: sum(is.na(fis.s$ppid))
#background info: gender & ethnicity, FT/PT & Transfer, RC

fig.fis_rc<-fis.s%>%
  ggplot(aes(x = RCstatus, y = `FT/PT`, fill = `Transfer YN`)) +
  geom_col(stat="identity")+ #是stakced
  #geom_col(position = position_dodge(stat=""identity""))#是毗邻的
  scale_fill_manual(...)+
  labs(title=str_wrap("",35),x="",y=""，fill="", subtitle="", caption="") #80 is a full line length
facet_wrap(~var,labeller=labeller(var= c(`oldname1`=""newname1"", ...) ) )"
