...
#create two bulk group and plot
data%>%filter(`Race/Ethnicity`!=c("Unreported",NA))%>%mutate(RaceGroup=case_when(
...
ggplot(aes(color=factor(`Race/Ethnicity`),x=factor(Year),y=prt,group=factor(`Race/Ethnicity`)))+geom_line()+
  scale_color_manual(name="",limits=nrace$race, values = c(mycolors[1],mycolors[2],mycolors[4],mycolors[6],mycolors[7],"green"))+  
...
geom_text_repel(aes(label=prt),fontface="bold",size=6.5)+
#themes
scale_y_continuous(breaks = seq(.2,.6,by=.2),labels = scales::percent)+
theme_lz_ppt()+theme(axis.text.y=element_blank())+theme(legend.position = "right", legend.direction = "vertical")+
