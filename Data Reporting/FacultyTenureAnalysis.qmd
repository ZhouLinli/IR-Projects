...
plot.dep<-function(min.input,max.input){
  ...
  theme(axis.text.y=element_text(size = 12),plot.title = element_text(size=18))+
  labs(x="",y="",title=str_wrap( 
    paste0("Faculty Tenure Status for Departments (",min.input, "<n<",max.input,")"),70))}
```
...
library(shiny)
inputPanel(
  sliderInput("min.input", label = "Department's minimum count of faculty:",
              min = 0, max = 147, value = 0, step = 1),
  sliderInput("max.input", label = "Department's maximum count of faculty:",
              min = 0, max = 147, value = 147, step = 1)
)

renderPlot({
plot.dep(min.input,max.input)
})
```
