...
```{r ethnicity/gender col and SAVE}
...
dmt0922.s<-dmt0922%>%select(PC_ID, ETHNICITY_REPORT_DESC,GENDER_CODE)%>%
  rename(ppid=PC_ID, ethnicity=ETHNICITY_REPORT_DESC,gender=GENDER_CODE)#consistency when merge
...
#save
write_xlsx(ipeds.cp,"/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/Completions/Complete.with.Ethnicity.xlsx")
```


```{r reporting}
ipeds.cp<-read_xlsx("/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/Completions/Complete.with.Ethnicity.xlsx")
...
#10-10-2022 adding 23.1302 MFA
ipeds.cp%>%filter(cip=="23.1302",Degree=="MFA")%>%
  group_by(ethnicity,gender,.drop = FALSE)%>%count()%>%
  pivot_wider(names_from = gender,values_from = n)%>%View()


# DID NOT REPORT 23.1302-MFA  AND 24.0000-AA

#24.0101, DO NOT HAVE ANY
...
ipeds.cp%>%filter(cip=="54.0101",Degree=="BA")%>%
  group_by(ethnicity,gender,.drop = FALSE)%>%count()%>%
  pivot_wider(names_from = gender,values_from = n)%>%View()
#54.0101, HOW TO REPORT SECOND MAJOR
```
...
## 52.0801
## 54.0101
###when deciding 1st/ 2nd major -- see all people with 2 majors, arbitrarily select one as 1st and the other as 2nd for all.

#DID NOT REPORT:
## 23.1302-MFA  
## 24.0000-AA -make it 24.0101?
## 43.0404-BS
## 51.0913-MSAT
## 52.1401-CER
