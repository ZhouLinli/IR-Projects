...
- Bayesian data analysis 
  ...
  - Bayesian focuses on the theoretical posterior probability distribution/ model parameters, a conditional probability, logically inferred (i.e. using the likelihood function ^[likelihood function: represents the probability of random variable realizations conditional on particular values of the statistical parameters.] or the Bayes's theorem/ inverse probability) based on 1) observed data and 2) prior prior probability distribution/ model parameters. Probability is degree of belief based on counting events, or updating model parameters. But application of Baysian methods remain computationally expensive, so alternatives/approximations to Baysian inference are valuable.
