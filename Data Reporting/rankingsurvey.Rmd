...
```{r no-option and sectiontally course data and SAVE}
...
#initiate needed vars
sec.s<-sec%>%select(event_id,section,Current_Enrollment)%>%
  mutate(CourseCode=paste(sep=".",event_id,section))%>%#same format
  select(-event_id,-section)%>%unique()%>%arrange(CourseCode)#remove used
#duplicated
sec.s%>%group_by(CourseCode)%>%summarise(cnt=n())%>%ungroup()%>%count(cnt)#each course appeared muti-times
#de-duplicate: only keep max values
sum(is.na(sec.s$Current_Enrollment))#there is no NA values (for arrange orders)
#then keep only the top for each coursecode
sec.s<-sec.s%>%arrange(desc(Current_Enrollment))%>%#largest at the top
  arrange(CourseCode)%>%#same id group and has large-low cumcredit
  distinct(CourseCode,.keep_all = TRUE)#distinct will keep the !first row!

#create merged usn.crs
usn.crs<-left_join(usn.crs,sec.s)%>%
...
#save
write.xlsx(usn.crs,"/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/US News/2022 fall/OnlineStudentCourse.Class.xlsx")
```


```{r report course.class}
#remove invalid size (current enrollment is na)
usn.crs<-usn.crs%>%filter(!is.na(Current_Enrollment))

#question: for each program, number of class (section) in range of different class size
usn.crs%>%count(Department,CourseCode,size)%>%
  select(-n)%>%#apperance of course code does not matter
  count(size,Department)%>%arrange(Department)

#question: for each program, mean class size
usn.crs%>%group_by(Department)%>%summarise(mn.size=mean(Current_Enrollment))
...
#retention is how many student is there in 22 data
