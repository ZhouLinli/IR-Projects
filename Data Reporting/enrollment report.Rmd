...
```{r ug.gd class/report order and SAVE}
#ug.gd%>%count(transfer)
ug.gd<-ug.gd%>%mutate(status=factor(status),
                     ...
                     level.c=factor(level.c,levels = c("UGdegree","UGnondeg","GDdegree","GDnondeg")))
...
t1.1<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
...
t1.2<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
...
t1.3<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
...
#NON-DEGREE
t1.4<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.c%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())

#merge TABLE 1
t1<-plyr::join_all(list(t1.1,t1.2,t1.3,t1.4),type="full")
t1[is.na(t1)]<-0#without count are NA, convert to 0
View(t1)#for convinience of copy-paste
```
...
#Table1.Full-time Undergraduate Students:MEN
#DEGREE: FIRST TIME (report first-time student in DEGREE-SEEKING column of the MEN table of FT UG)
t2.1<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Non-transfer")%>%#first time=first year at Lasell+first time at all
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, newly transfer in
t2.2<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Transfer")%>%#non-first-time transfer in=new+transfer
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, return (anyone who returns regardless their previously transfer students or not)
t2.3<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="return")%>%#returning (including previous transfer students)
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#NON-DEGREE
t2.4<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.c%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#merge TABLE 2
t2<-plyr::join_all(list(t2.1,t2.2,t2.3,t2.4),type="full")
t2[is.na(t2)]<-0#without count are NA, convert to 0
View(t2)#for convinience of copy-paste
```
...
#Table1.Full-time Undergraduate Students:MEN
#DEGREE: FIRST TIME (report first-time student in DEGREE-SEEKING column of the MEN table of FT UG)
t3.1<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Non-transfer")%>%#first time=first year at Lasell+first time at all
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, newly transfer in
t3.2<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Transfer")%>%#non-first-time transfer in=new+transfer
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, return (anyone who returns regardless their previously transfer students or not)
t3.3<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="return")%>%#returning (including previous transfer students)
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#NON-DEGREE
t3.4<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.c%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#merge TABLE 1
t3<-plyr::join_all(list(t3.1,t3.2,t3.3,t3.4),type="full")
t3[is.na(t3)]<-0#without count are NA, convert to 0
View(t3)#for convinience of copy-paste
```
...
#Table1.Full-time Undergraduate Students:MEN
#DEGREE: FIRST TIME (report first-time student in DEGREE-SEEKING column of the MEN table of FT UG)
t4.1<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Non-transfer")%>%#first time=first year at Lasell+first time at all
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, newly transfer in
t4.2<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Transfer")%>%#non-first-time transfer in=new+transfer
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, return (anyone who returns regardless their previously transfer students or not)
t4.3<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.c%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="return")%>%#returning (including previous transfer students)
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#NON-DEGREE
t4.4<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.c%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#merge TABLE 2
t4<-plyr::join_all(list(t4.1,t4.2,t4.3,t4.4),type="full")
t4[is.na(t4)]<-0#without count are NA, convert to 0
View(t4)#for convinience of copy-paste
```
...
t1<-ug.gd%>%filter(sourceLevel=="UG")%>%filter(`FT/PT`=="PT",Gender=="M",sourceLevel=="Degree-seeking",`College Attend`=="NEW")%>%
  group_by(ethnicity.c,.drop = FALSE)%>%summarise(firsttime=n())
...
t2<-ug.gd%>%filter(sourceLevel=="UG")%>%filter(`FT/PT`=="PT",Gender=="M",sourceLevel=="Degree-seeking", `College Attend`=="RET")%>%
  group_by(`Transfer YN`,ethnicity.c,.drop = FALSE)%>%count()%>%
...
t3<-ug.gd%>%filter(sourceLevel=="UG")%>%filter(`FT/PT`=="PT",Gender=="M",sourceLevel=="Non-degree")%>%
  group_by(ethnicity.c)%>%summarize(nondegree=n())
...
#GD MEN
##report MEN's ethnicity.c by gender
t<-ug.gd%>%filter(sourceLevel=="GD")%>%filter(Gender=="M")%>%group_by(ethnicity.c,`FT/PT`,.drop = FALSE)%>%count()%>%
...
#GD WOMEN
##report WOMEN's ethnicity.c by gender
t<-ug.gd%>%filter(sourceLevel=="GD")%>%filter(Gender=="F")%>%group_by(ethnicity.c,`FT/PT`,.drop = FALSE)%>%count()%>%
  pivot_wider(names_from = `FT/PT`, values_from = n)
t$FT[is.na(t$FT)]<-0
t$PT[is.na(t$PT)]<-0
View(t)
