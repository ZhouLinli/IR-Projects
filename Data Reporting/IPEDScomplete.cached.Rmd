...
```{r ipeds.cp}
#select vars needed
ipeds.cp<-rgs.cp%>%select(ppid, Program, Major,cip,`Graduation Date`)%>%unique()#547 rows
...
###merge
ipeds.cp.t<-plyr::join_all(list(ipeds.cp, dmt0922.s),type = "left")%>%unique()#597>574
# duplication: ipeds.cp.t%>%group_by(ppid)%>%count()%>%ungroup()%>%count(n)# have 49 dup
dup.id<-ipeds.cp.t%>%group_by(ppid)%>%count()%>%ungroup()%>%filter(n>1)
dup<-ipeds.cp.t%>%filter(ppid %in% dup.id$ppid)
#known in the duplicated df
eth.k<-dup%>%filter(ethnicity!="Race and Ethnicity Unknown")%>%unique()
#unknown in the duplicated df
eth.unk<-dup%>%filter(ethnicity=="Race and Ethnicity Unknown")%>%unique()%>%arrange(ppid)
#compare
sum(! eth.unk$ppid%in%eth.k$ppid)# all unknown are in known
#ppid that have both/intersect, keep the eth.k one
eth.int<-inner_join(x=eth.k, y=eth.unk, by="ppid")%>%
  mutate(ethnicity=ethnicity.x)%>%
  select(ppid,ethnicity)%>%arrange(ppid)
#ppid that have no intersect, they are truely ethnicity.unknown
eth.unk<-eth.unk%>%filter(! ppid%in% eth.int$ppid)
  


#merge: filtered truely eth.unk and the other eth.k (from duplication df)
dup.t<-plyr::join_all(list(eth.unk, eth.k), type = "full")%>%unique()%>%arrange(ppid)




###solve ethnicity conflict by looking up the academic year and term
