...
```{r corepath merge background info}
#merge more background vars
corepath.bk<-left_join(corepath,dmt[c("PC_ID","MATRIC_YT_YN")],by=c("people_code_id"="PC_ID"))

#any of the term is “1” then they are
y<-corepath.bk%>%mutate(matric=case_when(MATRIC_YT_YN=="1"~"Graduated",
                                         MATRIC_YT_YN!="1"~"Not Graduate"))%>%
  select(people_code_id,matric)%>%unique()%>%arrange(people_code_id)%>%
  filter(matric =="Graduated")#keep any grad values of new col

n<-corepath.bk%>%filter(! people_code_id%in% y$people_code_id)%>% #for people_code_id with not settled
...
corepath.bk<-left_join(corepath.bk,matric.corepath.bk)%>%
  select(-MATRIC_YT_YN)%>% unique()%>%arrange(people_code_id)#remove used MATRIC_YT_YN (used matric replace MATRIC_YT_YN)

corepath.bk%>%count(matric)
