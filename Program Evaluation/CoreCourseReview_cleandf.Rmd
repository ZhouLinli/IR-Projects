...
4.  stucrs_1822allcrs: merging 18-22 fall and spring files from "student
    info by course" files, only keep valid grades in "Final_Grade", only
    keep valid (\>0) "Semester_GPA", only keep active students (not
...
10. aac103retake: based on aac.crs, filter out those who retake aac103
...
11. retake.change: "passfail" and "semGPA" for each ppid (row) each term
    ...
    write_xlsx(aac103retake,"/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Data and Analyses/Program Review/2022 AAC/data files/aac103retake_stuinfo.xlsx")
...
GPA=as.numeric(GPA))%>%
# only kep valid semester GPA
filter(Semester_GPA>0)%>%
...
mutate(semGPA=case_when(Semester_GPA>=2~"higher than 2",Semester_GPA<2~"lower than 2"))#check: %>%count(semGPA)
