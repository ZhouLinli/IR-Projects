...
resp_rate<-allstu22.s%>%group_by(ResAll)%>%summarise(cnt=n())%>%
 mutate(prt=round(cnt/sum(cnt)*100,digits = 0))%>%
  ...
  scale_fill_manual(values = c("#6C6F70","#69b3e7"))+
...
current<-rbind(connect.t1,connect.t2,connect.t3,
      ...
      satis.t1,satis.t2,satis.t3,satis.t4,satis.t5,satis.t6,satis.t7,satis.t8,satis.t9,
...
prtdf%>%filter(item%in%
           c("satis.overallcourse","satis.majorcourse", "satis.acchallenge", "satis.accommit"),
           year>2019)%>%
  group_by(item,year,prt)%>%summarise(cnt=n())%>%
  
  ggplot(aes(y = prt, fill=year, x=factor(item,levels =c("satis.overallcourse","satis.majorcourse", "satis.acchallenge", "satis.accommit") )))+
  geom_bar(stat="identity",position = position_dodge(),width=0.8)+
  #labs(title=str_wrap("",40),x="",y="",fill="") +
  #theme_lz()+theme(axis.text.x = element_text(),axis.text.y = element_text(), legend.position = c(0.8,0.8))+
  ...
  scale_y_continuous(breaks = seq(0.7, 1, by = 0.1) ,
                     labels=scales::percent_format(accuracy = 1))+#present y axis label as percentage
  scale_x_discrete(labels=c("Overall Courses","Major Courses","Academic Challenge","Academic Commitment"),
                   guide=guide_axis(n.dodge = 1))+
  scale_fill_manual(values = c("Grey","Grey70","#69b3e7"))#fill legend labels=c(""),name="")
