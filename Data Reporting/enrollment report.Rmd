...
sts.fall<-ug.gd%>%group_by(ppid)%>%
  mutate(status=if_else(term=="21fall",as.character(`FT/PT`),"No fall"))%>%
  filter(status%in%c("FT","PT"))%>%arrange(ppid)#only keep the settled
...
sts.spring<-ug.gd%>%group_by(ppid)%>%
  ...
  arrange(ppid)%>%
  filter(status%in%c("FT","PT"))#only keep the newly settled
...
sts.winter<-ug.gd%>%group_by(ppid)%>%
  ...
  arrange(ppid)%>%
  filter(status%in%c("FT","PT"))
#not in fall&spring&winter, then determine by summer2 FT/PT  
sts.summer2<-ug.gd%>%group_by(ppid)%>%
  filter(! ppid%in% c(sts.fall$ppid,
                      sts.spring$ppid,
                      sts.winter$ppid))%>%#only care those not-settled (after two settled cohort)
...
sts<-plyr::join_all(list(sts.fall,sts.spring,sts.summer2,sts.summer1,sts.winter),type ="full" )%>% 
  select(ppid,status)%>%#select only relevant
...
#if not in fall at all, determine by spring
ret.spring<-ug.gd%>%group_by(ppid)%>%
  filter(! ppid%in% ret.fall$ppid)%>%#if do not show up in fall
  mutate(NewRet=if_else(term=="22spring",as.character(`New Ret Term YN`),"No spring"))%>%
  arrange(ppid)%>%
  filter(NewRet%in%c("new","return"))#either new or return of fall would be settled
...
ret.summer2<-ug.gd%>%group_by(ppid)%>%
  filter(! ppid%in% c(ret.fall$ppid,
                      ret.spring$ppid,
                      ret.winter$ppid))%>%#if do not show up in fall&spring
...
deg.s<-ug.gd%>%group_by(ppid,degree.t)%>%
  ...
  filter(degree.c=="Degree-seeking")#keep any degree-seeking
