...
#grouping ndep
mylist1<-ndep[ndep$cnt<5,1]$Department
mylist2<-ndep[ndep$cnt%in%seq(5,20,by=1),1]$Department
mylist3<-ndep[ndep$cnt%in%seq(21,30,by=1),1]$Department
mylist4<-ndep[ndep$cnt>30,1]$Department

#plot for each group
plot.dep<-function(mylist){
  ...
  theme(axis.text.y=element_text(size = 12),plot.title = element_text(size=14))+
  labs(x="",y="",title=str_wrap( 
    paste0("Faculty Tenure Status for Departments (",min(mylist), "<n<",max(mylist)),35))}
