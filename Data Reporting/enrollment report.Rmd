...
# consitency: first full term (fall spring winter summer)
...
lev.g<-ug.gd%>%group_by(ppid,level.degree)%>%summarise(cnt=n())%>%#unique output
  mutate(level.deg=if_else(level.degree %in% c("GDdegree"),as.character(level.degree),"non"))%>%arrange(ppid)%>%
  filter(level.deg != "non")
...
lev.u<-ug.gd%>%filter(! ppid%in% lev.g$ppid)%>%#not settled with lev.s
  ...
  mutate(level.deg=if_else(level.degree %in% c("UGdegree"),as.character(level.degree),"non"))%>%arrange(ppid)%>%
  filter(level.deg != "non")
...
lev.n<-ug.gd%>%filter(! ppid%in% c(lev.g$ppid,lev.u$ppid))%>%#not settled
  mutate(level.deg=if_else(level.degree %in% c("GDnondeg","UGnondeg"),as.character(level.degree),"non"))%>%arrange(ppid)%>%
  filter(level.deg != "non")# for not appear once as deg, keep them as GD/UGnondeg

#merge settled lev
lev<-plyr::join_all(list(lev.g,lev.u,lev.n),type ="full" )%>% 
  select(ppid,level.deg)%>%
 unique()%>%arrange(ppid)#2057
...
```{r mutate and factorize and SAVE}
##########add ug/gd credit separately
##########AGGREGATE term to total UG/GD credit 
...
tot.crd<-ug.gd%>%group_by(ppid,sourceLevel)%>% #we only care about total for each student
  ...
  pivot_wider(names_from = sourceLevel,values_from = tot.credit)
...
ug.gd<-ug.gd%>%mutate(status=factor(status),
                      ...
                      level.deg=factor(level.deg,levels = c("UGdegree","UGnondeg","GDdegree","GDnondeg")),
                      level=case_when(level.deg%in%c("UGdegree","UGnondeg")~"UG",
                                      level.deg%in%c("GDdegree","GDnondeg")~"GD"),
                      level=factor(level,levels = c("UG","GD")))
#save all
write.xlsx(ug.gd,file="/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/12-month enrollment/ug.gd_21-22_rerun.xlsx")
```
...
#check: ipeds%>%group_by(ppid)%>%summarise(cnt=n())%>%ungroup()%>%group_by(cnt)%>%count()
...
#ipeds%>%count(Gender)
ipeds%>%group_by(NewRet,transfer)%>%count()

#save all
write.xlsx(ipeds,file="/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/12-month enrollment/ipeds_21-22_rerun.xlsx")
```
...
ug.t1.1<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
...
ug.t1.2<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
...
ug.t1.3<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
...
#NON-DEGREE
ug.t1.4<-ipeds%>%filter(status=="FT",#full time
               Gender=="M",#male
               level.deg%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#Table1.Full-time Undergraduate Students:MEN
#DEGREE: FIRST TIME (report first-time student in DEGREE-SEEKING column of the MEN table of FT UG)
ug.t2.1<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Non-transfer")%>%#first time=first year at Lasell+first time at all
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, newly transfer in
ug.t2.2<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Transfer")%>%#non-first-time transfer in=new+transfer
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, return (anyone who returns regardless their previously transfer students or not)
ug.t2.3<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="return")%>%#returning (including previous transfer students)
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#NON-DEGREE
ug.t2.4<-ipeds%>%filter(status=="FT",#full time
               Gender=="F",#male
               level.deg%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#Table1.Full-time Undergraduate Students:MEN
#DEGREE: FIRST TIME (report first-time student in DEGREE-SEEKING column of the MEN table of FT UG)
ug.t3.1<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Non-transfer")%>%#first time=first year at Lasell+first time at all
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, newly transfer in
ug.t3.2<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Transfer")%>%#non-first-time transfer in=new+transfer
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, return (anyone who returns regardless their previously transfer students or not)
ug.t3.3<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="return")%>%#returning (including previous transfer students)
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#NON-DEGREE
ug.t3.4<-ipeds%>%filter(status=="PT",#full time
               Gender=="M",#male
               level.deg%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#Table1.Full-time Undergraduate Students:MEN
#DEGREE: FIRST TIME (report first-time student in DEGREE-SEEKING column of the MEN table of FT UG)
ug.t4.1<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Non-transfer")%>%#first time=first year at Lasell+first time at all
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, newly transfer in
ug.t4.2<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="new",transfer=="Transfer")%>%#non-first-time transfer in=new+transfer
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#DEGREE: NON-FIRST-TIME, return (anyone who returns regardless their previously transfer students or not)
ug.t4.3<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.deg%in%c("UGdegree"),#undergrad degree seeking
               NewRet=="return")%>%#returning (including previous transfer students)
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
...
#NON-DEGREE
ug.t4.4<-ipeds%>%filter(status=="PT",#full time
               Gender=="F",#male
               level.deg%in%c("UGnondeg"))%>%#undergrad non degree
    group_by(ethnicity.c,.drop = FALSE)%>%#appear groups that has zero count
    summarise(nondeg=n())
...
#full time gd men
gd.t1.1<-ipeds%>%filter(Gender=="M",#male
               level.deg%in%c("GDdegree","GDnondeg"),#all gd students
               status=="FT")%>%#full time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(ft.gd.m=n())
#part time gd men
gd.t1.2<-ipeds%>%filter(Gender=="M",#male
               level.deg%in%c("GDdegree","GDnondeg"),#all gd students
               status=="PT")%>%#part time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(pt.gd.m=n())
...
#GD MEN's ethnicity.c by gender
#full time gd men
gd.t2.1<-ipeds%>%filter(Gender=="F",#female
               level.deg%in%c("GDdegree","GDnondeg"),#all gd students
               status=="FT")%>%#full time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(ft.gd.f=n())
#part time gd men
gd.t2.2<-ipeds%>%filter(Gender=="F",#female
               level.deg%in%c("GDdegree","GDnondeg"),#all gd students
               status=="PT")%>%#part time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(pt.gd.f=n())
...
dk.t<-ipeds%>%filter(Gender=="U")%>%##gender unknown
...
```{r credit total}
crd.t<-ipeds%>%group_by(level)%>%#by UG/GD
        summarise(crd=sum(tot.credit))#sum tot.credit (total term credit each ppid take for the whole year)
crd.t[is.na(crd.t)]<-0#without count are NA, convert to 0
View(crd.t)
```

# distance education question
```{r load course data}
...
crs<-plyr::join_all(list(crs21sum2,crs21fall,crs21winter,crs22spring,crs22sum1.main),type = "full")%>%
  ...
  select(ppid,event_type)%>%#the event_type is what we only need from this dataset
...
#save merged course file
write.xlsx(crs,file="/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/12-month enrollment/course_21-22.xlsx")
```

```{r course_type recode}
#recode event type to correspond with ipeds types
...
# Merged crs to ipeds
```{r merge crs type to ipeds enrolled students}
crs.ipeds<-plyr::join_all(list(ipeds,crs),type="left")%>%#all enrolled 2057 students defined in ipeds
  select(ppid,Name,merged.type,level.deg)#selected need for reports (type by level and degree)

#find course names of ppid that miss type values
type.missing<-crs.ipeds%>%filter(is.na(merged.type))%>%arrange(ppid)#36 enrolled students do not have a corresponding type
#check if the same with students who are not in ipeds (enrolled backup data)
#enroll.missing<-ipeds%>%filter(ppid%in% type.missing$ppid)%>%select(ppid,level.deg,status)%>%arrange(ppid)
...
missing.crsname<-read_csv("/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/12-month enrollment/distanceEd/Student Info by Course and Fiscal Year.csv") %>% 
  filter(people_code_id %in% type.missing$ppid) %>%#only concerned about those missing ppid
...
missing.crstype<-plyr::join_all(list(missing.crsname,sectally),type="left")%>%unique()
```


```{r standard.crstypes for merging conflicting types}
#missing.crstype%>%count(Course_type)
#fixing conflicting types values
standard.types_single<-missing.crstype%>%filter(! Course_type=="Course" | ! is.na(Course_type))%>%#set aside na values
...
standard.types_multi<-missing.crstype%>%filter(! Course_type=="Course" | ! is.na(Course_type))%>%#set aside na values
...
#for single type, keep its type; 
#the multi-types (from ONLN,CAMP,HYB), any of those types combined, should all be HYB
standard.crstype<-missing.crstype%>%
  mutate(merged.type=if_else(people_code_id %in% standard.types_single$people_code_id,
                                      Course_type,#keep single type's type
                      if_else(people_code_id %in% standard.types_multi$people_code_id,
                              "HYB",#all multi-type should be hyb
                              "not standard type")))%>%#not settled by single/multi standard types
          unique()#always remember this remove exact same rows, 250 rows
#standard.crstype%>%count(merged.type)

#recode
standard.crstype<-standard.crstype%>%mutate(type=case_when(merged.type %in% c("Online Course")~"ONLN",#recode
...
crstype.found<-standard.crstype%>%
  ...
  unique() #remove recoded old var, remove multi-coursenames for ppid, 32 rows

#not found
type.missing [! type.missing$ppid %in% crstype.found$people_code_id,] #P000056440 "P000056986" "P000057211" "P000057212"
# type.missing$ppid %in% missing.crsname$people_code_id not even in the course info by fiscal year file (to find course names in order to look up course type in section tally)

standard.crstype%>%filter(type=="Unspecified")%>%select(-merged.type,-type) # P000056440

```

```{r}
crs.ipeds<-plyr::join_all(list(crs.ipeds, crstype.found),type="left")%>%#all 
  select(ppid,merged.type,level.deg)#selected need for reports (type by level and degree)
```
...
```{r U}
#complete file: august 21  december 21 may 22
