...
conflict.credit<-dtm.rgs%>%mutate(diff=ifelse(`Term Credits.rgs` == SESS_REG_CREDITS.dtm, "matched","not match")) %>%
  filter(diff=="not match")%>%
  select(ppid,Name.rgs,`Term Credits.rgs`,SESS_REG_CREDITS.dtm)
...
write.xlsx(list("MissingID"=conflict.id,
           ...
           file="/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Registrar Reports/2022 Fall/datafreezing.xlsx")
```
