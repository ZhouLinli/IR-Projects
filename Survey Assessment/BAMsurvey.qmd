...
```{r report_act22}
bam.long%>%count(act22,act22_value)%>%
  ...
  geom_text(aes(label=percent(prt,digits = 0)),size=3,vjust=0.5,hjust=0.7,fontface="bold",position = position_dodge(0.9))+
  #geom_text(aes(label=if_else(prt>.8, as.character(round(prt,2)),"")),hjust=1.5,size=3,fontface="bold",position = position_dodge(0.9))+geom_text(aes(label=if_else(prt<.8, as.character(round(prt,2)),"")),hjust=1.5,
    labs(title="",x="",y="")+theme_lz()+theme(axis.text.y = element_text(),legend.position = "bottom/top")
```

## Participation Time

```{r report_act_time}
bam%>%#check: count(act_time)
...
ggplot(aes(y = act_time, x=factor(0))) + geom_boxplot(fill="#69b3e7") +
  ...
  labs(title="",x="Hours of activity participation",y="")+
...
ggplot(aes(x=2, y = prt, fill=act_future)) + geom_bar(stat="identity") +
  ...
  annotate("text",x = 0.5, y = 0.5, fontface="bold", label = str_wrap( "90% Continue Participation", width=10))+
  annotate("text",x = 2, y = 0.75,vjust=0,hjust=-0.5, fontface="bold", label = str_wrap( "10%", width=10))+
  annotate("text",x = 0.5, y = 0.75,vjust=0,hjust=4.5, fontface="bold", label = str_wrap( "37%", width=10))+
  annotate("text",x = 0.5, y = 0.75,vjust=0,hjust=-4, fontface="bold", label = str_wrap( "53%", width=10))+
  #0.5 is centered location+
  #annotate("text",x = 0, y = 0.5, fontface="bold", label = ifelse(status=="Complete",as.character(cnt),""))+#0.5 is centered location+
  scale_fill_manual(values = c("#69b3e7",color_blue_lasell,"grey"))+
  theme_lz()+labs(title="",x="",y="",fill="",subtitle = "")
```

## Benefits of Participating BAM Activities
```{r report_impved}
bam.long%>%#check: count(impved)
...
## Perceptions of BAM Benefits
```{r report_willimpv}
bam.long%>%#check:count(willimpv_value)
...
ggplot(aes(x=2, y = prt, fill=dem_gender)) + geom_bar(stat="identity") +
  ...
  annotate("text",x = 0.5, y = 0.75,vjust=0,hjust=4.5, fontface="bold", label = str_wrap( "61%", width=10))+
  annotate("text",x = 0.5, y = 0.75,vjust=0,hjust=-4, fontface="bold", label = str_wrap( "35%", width=10))+
  #0.5 is centered location+
  #annotate("text",x = 0, y = 0.5, fontface="bold", label = ifelse(status=="Complete",as.character(cnt),""))+#0.5 is centered location+
...
ggplot(aes(x=2, y = prt, fill=dem_race)) + geom_bar(stat="identity") +
  xlim(0.5, 2.5)+ coord_polar(theta="y",start=0)+ #circled coord, pie centered on 2; show .5-2.5 of pie (0-.5 hole)
  scale_fill_manual(values = c(color_blue_lasell,"grey","#69b3e7"))+
  #geom_text(aes(label=paste0(prt,"%")), size=3,vjust=-2,hjust=3,fontface="bold",position = position_dodge(1))+
  annotate("text",x = 0.5, y = 0.5, fontface="bold", label = str_wrap( "Race", width=10))+
  annotate("text",x = 2, y = 0.75,vjust=0,hjust=1, fontface="bold", label = str_wrap( "3%", width=10))+
  annotate("text",x = 2, y = 0.75,vjust=0,hjust=2.5, fontface="bold", label = str_wrap( "3%", width=10))+
  annotate("text",x = 0.5, y = 0.75,vjust=0,hjust=-3.5, fontface="bold", label = str_wrap( "94%", width=10))+
  #0.5 is centered location+
  #annotate("text",x = 0, y = 0.5, fontface="bold", label = ifelse(status=="Complete",as.character(cnt),""))+#0.5 is centered location+
