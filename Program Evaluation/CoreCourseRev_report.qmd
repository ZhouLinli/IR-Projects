...
col_cf<-function(i=1,kbl=static_tab.numsec,df=tab.numsec){column_spec(
  kable_input=kbl,column=i,color = "white",background =cf(df[i]))}
```

```{r static_tab.numsec}
#show the table
static_tab.numsec<-tab.numsec%>%adorn_totals()%>%#add total row this won't saved in df
kbl(label="tab.numsec",#in-text reference
...
#apply col_cf to all columns
for (i in 3:nrow(tab.numsec)-1){
  static_tab.numsec<-col_cf(i)
  i=i+1
  static_tab.numsec}

static_tab.numsec<- static_tab.numsec%>%
#set total col
column_spec(2, background =cf(x=tab.numsec[2],a=169,b=148,col1 =color_yellowdark,col2=color_greylight,col3=color_grey))%>%
#set total row
row_spec(nrow(tab.numsec)+1,color="black",background = "white")%>%
#set header row
...
```{r}
tab.numsec%>%pivot_longer(cols = `2018 Fall`:`2022 Spring`,names_to = "term",values_to = "#section")%>%
