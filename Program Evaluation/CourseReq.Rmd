...
pg
#search using css class as nodes
pg %>% html_nodes("a.mp-modal")%>%html_text()#we must add a . before the class name
```
...
#somehow the html course only matches the first 11 and the rest are below
html.nexist<-c("BUSS203","BUSS208","BUSS211","BUSS226","BUSS310","CFP302X","CFP303X","ECON102","MATH202","BUSS307","BUSS331")
```
...
#there are coures that are not output in the html (even though the web inspection shows a.mp-modal)
new.html.nexist<-html.nexist[(html.nexist%in%dat.major$Section) == FALSE]%>%as.data.frame()

#append new.df with dat.major
dat.major<-plyr::join_all(list(dat.major, new.crs.df, new.html.nexist),type = "full")# add one 954
```
...
dat.major<-
  dat.major%>%mutate(`Esports-and-Gaming-Management`=case_when(
   # `Esports-and-Gaming-Management`=="C"~"Core", #hide for not-existing major
    Section %in% crs.df$Section[1:11] ~"Required",#somehow the html course only matches the first 11
    Section %in% html.nexist ~"Required",
    ))#all required course

#check
dat.major%>%group_by(`Esports-and-Gaming-Management`)%>%count()
```
...
#append new.df with dat.major
dat.major<-plyr::join_all(list(dat.major,new.crs.df),type = "full")# 948 to 953
```
...
dat.major<-
  dat.major%>%mutate(`Corporate-Finance`=case_when(
   # `Corporate-Finance`=="C"~"Core", #hide for not-existing major
    ...
    Section %in% crs.df$Section[20:length(crs.df$Section)] ~"Elective"))#%>%group_by(`Corporate-Finance`)%>%count()#check directly after mutate, before assign it to df

#check
dat.major%>%group_by(`Corporate-Finance`)%>%count()
```