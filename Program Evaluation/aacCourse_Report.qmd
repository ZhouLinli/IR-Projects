...
enrollment for AAC103 over that time is
...
Average pass rate for all AAC courses are above
`r mean(tab.aacpass_avg$Average)%>%percent(digit=0)` across the past 5
academic years, aside from AAC103 in 2020 Fall in which there was a 59%
pass rate.
...
retaking the course for the second and third time. In other words, AAC
...
-   Taking AAC 103 the second and third times does not improve pass
    ...
    `r tab.retakepass[3,"Pass rate"]%>%percent(digit=0)` when students
    retaking the course for the second and third time.
