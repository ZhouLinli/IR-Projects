...
tab.biocnt<-stucrs.si%>%filter(grepl("^BIO",coursecode))%>%
  ...
  summarise(n=n())%>%mutate(prt=n/sum(n))%>%select(-n)%>%
  filter(prt>.01 & !is.na(Major))%>%
  pivot_wider(names_from=coursecode,values_from=prt,values_fill=0)
tab.biocnt<-tab.biocnt%>%
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^BIO",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0

for (i in 2:ncol(tab.biocnt)){
viz.biocnt<-
column_spec(kable_input=viz.biocnt,column=i, width = "4em",color="white",
            background =cf_color_blue(tab.biocnt[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.biocnt}
```
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^CHEM",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0
...
column_spec(kable_input=viz.chemcnt,column=i, width = "4em",color="white",
            background =cf_color_blue(tab.chemcnt[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.chemcnt}
```
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^ENV",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0
...
column_spec(kable_input=viz.envcnt,column=i, width = "4em",color="white",
            background =cf_color_blue(tab.envcnt[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.envcnt}
```
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^PHYS",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0
...
column_spec(kable_input=viz.physcnt,column=i, width = "4.5em",color="white",
            background =cf_color_blue(tab.physcnt[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.physcnt}
```
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^SCI10",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0
...
column_spec(kable_input=viz.scicnt10,column=i, width = "3em",color="white",
            background =cf_color_blue(tab.scicnt10[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.scicnt10}
```
...
```{r scicnt11}
#count major (with toal row&col) for each coursecode across 18-22
tab.scicnt11<-stucrs.si%>%filter(grepl("^SCI114|SCI118",coursecode))%>%
  group_by(coursecode,Major)%>%
  summarise(n=n())%>%mutate(prt=n/sum(n))%>%select(-n)%>%
  filter(prt>.01 & !is.na(Major))%>%
  pivot_wider(names_from=coursecode,values_from=prt,values_fill=0)
tab.scicnt11<-tab.scicnt11%>%
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^SCI11",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0
...
column_spec(kable_input=viz.scicnt11,column=i, width = "4em",color="white",
            background =cf_color_blue(tab.scicnt11[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.scicnt11}
```

```{r show.scicnt11, fig.width=8,include=TRUE}
#| label: tbl-scicnt11
#| tbl-cap: "Distributions of Majors for SCI114 and 118 Courses"
viz.scicnt11
```

```{r scicntsingle}
#count major (with toal row&col) for each coursecode across 18-22
tab.scicntsingle<-stucrs.si%>%filter(grepl("^SCI115",coursecode))%>%
  group_by(coursecode,Major)%>%
  summarise(n=n())%>%mutate(prt=n/sum(n))%>%select(-n)%>%
  filter(prt>.01 & !is.na(Major))%>%
  pivot_wider(names_from=coursecode,values_from=prt,values_fill=0)
tab.scicntsingle<-tab.scicntsingle%>%
  mutate(across(names(tab.scicntsingle)[-1], function(x) percent(x, digit=0)))  #make numbers percentage

#format table as viz (format beautifully with kbl functions)
viz.scicntsingle<-tab.scicntsingle%>%
kbl(align = "c",booktabs = T, col.names =c("",str_wrap(names(tab.scicntsingle)[2:ncol(tab.scicntsingle)],width = 3))) %>%
#set in-text reference label, table title , header names
kable_styling(latex_options = c('HOLD_position'), full_width = F, fixed_thead = T, font_size = 12,bootstrap_options = c("striped", "hover", "condensed", "responsive"))%>%
#set overall style
column_spec(1, bold = F, border_right = F, background = "white", width = "20em",color="black")
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^SCI115",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0

for (i in 2:ncol(tab.scicntsingle)){
viz.scicntsingle<-
column_spec(kable_input=viz.scicntsingle,column=i, width = "4em",color="white",
            background =cf_color_blue(tab.scicntsingle[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.scicntsingle}
```

```{r show.scicntsingle, fig.width=8,include=TRUE}
#| label: tbl-scicntsingle
#| tbl-cap: "Distributions of Majors for SCI115 Courses"
viz.scicntsingle
```
...
#conditional formatting
cutraw<-stucrs.si%>%filter(grepl("^CJ",coursecode))%>%group_by(coursecode,Major)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%ungroup()%>%select(prt)
cut1<-(max(cutraw)-min(cutraw))*.6+min(cutraw)
cut2<-(max(cutraw)-min(cutraw))*.2+min(cutraw)
cut3<-(max(cutraw)-min(cutraw))*.1+min(cutraw)
cut4<-0
...
column_spec(kable_input=viz.cjcnt,column=i, width = "4em",color="white",
            background =cf_color_blue(tab.cjcnt[i],a=cut1,b=cut2,c=cut3,d=cut4))
i=i+1; viz.cjcnt}
```
