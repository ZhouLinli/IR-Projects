...
# ONE DOMINATE: UG, then GD, then not any UG/GD then non-degree
```{r fix non-deg level and level consistency}
#need to have UG/GD for even non-matriculated students; based on program names

#non-degree shouldn't have any level
ug.gd<-ug.gd%>%mutate(Level=if_else(
  degree.t=="Non-degree","Non-degree",as.character(Level)))%>%#recode non-degree level
  select(-degree.t)#can refer degree to level, no longer need degree
...
#level should be determined by the program name
ls<-ug.gd%>%group_by(Program)%>%count()%>%arrange(Program); View(ls)
...
#for ppid with not any degree-seeking
lev.g<-ug.gd%>% filter(! ppid%in% c(lev.u$ppid))%>%#for any not settled
  mutate(level.c=if_else(Level=="GD","GD","non"))%>%
  arrange(ppid)%>%filter(level.c=="GD")%>%unique()#keep GD

#not any UG/GD status, then non-degree
lev.n<-ug.gd%>%filter(! ppid%in% c(lev.u$ppid, lev.g$ppid))%>%
...
ug.gd<-plyr::join_all(list(ug.gd, lev),type="left")%>%
  select(-Level)%>%unique()%>%arrange(ppid)#only what we needed, 4422; do not need degree-seeking any more, because that will be determined by level
```
...
t1<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="FT",Gender=="M",Level=="Degree-seeking",`College Attend`=="NEW")%>%
...
t2<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="FT",Gender=="M",Level=="Degree-seeking", `College Attend`=="RET")%>%
...
t3<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="FT",Gender=="M",Level=="Non-degree")%>%
...
##########DEGREE: FIRST TIME (College Attend=NEW)
##report first-time student in DEGREE-SEEKING column of the MEN table of UG
t1<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="FT",Gender=="F",Level=="Degree-seeking",`College Attend`=="NEW")%>%
  group_by(Ethnicity,.drop = FALSE)%>%summarise(firsttime=n())

##########DEGREE&NON FIRST TIME: TRANSFER VS RETURNING in "Transfer YN"
## focusing on return student, whether they are transfer or not
t2<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="FT",Gender=="F",Level=="Degree-seeking", `College Attend`=="RET")%>%
  group_by(`Transfer YN`,Ethnicity,.drop = FALSE)%>%count()%>%
  #transferY&N side by side
  pivot_wider(names_from = `Transfer YN`,values_from =  n, names_glue = paste0("Transfer","{`Transfer YN`}_{.value}"))
...
##########NON DEGREE
#report NON-DEGREE column
t3<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="FT",Gender=="F",Level=="Non-degree")%>%
  group_by(Ethnicity)%>%summarize(nondegree=n())
...
##########DEGREE: FIRST TIME (College Attend=NEW)
##report first-time student in DEGREE-SEEKING column of the MEN table of UG
t1<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="PT",Gender=="M",Level=="Degree-seeking",`College Attend`=="NEW")%>%
  group_by(Ethnicity,.drop = FALSE)%>%summarise(firsttime=n())

##########DEGREE&NON FIRST TIME: TRANSFER VS RETURNING in "Transfer YN"
## focusing on return student, whether they are transfer or not
t2<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="PT",Gender=="M",Level=="Degree-seeking", `College Attend`=="RET")%>%
  group_by(`Transfer YN`,Ethnicity,.drop = FALSE)%>%count()%>%
  #transferY&N side by side
  pivot_wider(names_from = `Transfer YN`,values_from =  n, names_glue = paste0("Transfer","{`Transfer YN`}_{.value}"))
...
##########NON DEGREE
#report NON-DEGREE column
t3<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="PT",Gender=="M",Level=="Non-degree")%>%
  group_by(Ethnicity)%>%summarize(nondegree=n())
...
##########DEGREE: FIRST TIME (College Attend=NEW)
##report first-time student in DEGREE-SEEKING column of the MEN table of UG
t1<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="PT",Gender=="F",Level=="Degree-seeking",`College Attend`=="NEW")%>%
  group_by(Ethnicity,.drop = FALSE)%>%summarise(firsttime=n())

##########DEGREE&NON FIRST TIME: TRANSFER VS RETURNING in "Transfer YN"
## focusing on return student, whether they are transfer or not
t2<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="PT",Gender=="F",Level=="Degree-seeking", `College Attend`=="RET")%>%
  group_by(`Transfer YN`,Ethnicity,.drop = FALSE)%>%count()%>%
  #transferY&N side by side
  pivot_wider(names_from = `Transfer YN`,values_from =  n, names_glue = paste0("Transfer","{`Transfer YN`}_{.value}"))
...
##########NON DEGREE
#report NON-DEGREE column
t3<-ug.gd%>%filter(Level=="UG")%>%filter(`FT/PT`=="PT",Gender=="F",Level=="Non-degree")%>%
  group_by(Ethnicity)%>%summarize(nondegree=n())
