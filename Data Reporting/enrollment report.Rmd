...
ug.gd<-plyr::join_all(list(ug.gd.t,deg),type = "left")%>%select(-degree)#4422
```
...
crs.ipeds<-plyr::join_all(list(ipeds,crs),type="left")%>%#all enrolled 2057 students defined in ipeds
  select(ppid,Name,merged.type,level,degree)#selected need for reports (type by level and degree)
...
crs.ipeds<-plyr::join_all(list(crs.ipeds, crstype.found),type="left")%>%#all 
  select(ppid,merged.type,level,degree)#selected need for reports (type by level and degree)
```
