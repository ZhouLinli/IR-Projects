...
alum<-alum%>%mutate(Employment_2=case_when(
  Employment %in% c("Additional Eduation","Additional Education","Edu","Further Ed","n/a","Other","UNK","Unknown") ~ "Unknown",
  Employment %in% c("Contract","Freelance","Internship") ~ "Employed",
  str_detect(alum$Employment, "^[Ee][Mm][Pp]")~ "Employed",#start with emp no matter upper/lower case
  Employment %in% c("No Emp No Edu","Unemployed")~"Unemployed"
))

#check
alum%>%group_by(Employment,Employment_2)%>%count()#NA cannot be recoded
