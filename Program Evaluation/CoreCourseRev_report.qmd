...
```{r sectally - number of section offered}
...
tab.numsec<-sectiontally%>%group_by(CourseGroup,term)%>%summarise(section_n=n())%>%
...
cond_secnum<-function(x){
    if_else(x>20,color_bluedeepsea,
      if_else(x>10,color_bluelightsky,color_grey))}

#show the table
static_tab.numsec<-tab.numsec%>%kbl(label="tab.numsec",caption = "Core course section offered each term",
                 col.names =c("",names(tab.numsec)[2:ncol(tab.numsec)])) %>%#table title
#set overall style
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), #the same as: kable_material(c("striped", "hover")) #less than 3 row then use: kable_minimal()
                full_width = F, position = "float_left",font_size = 10,
                fixed_thead = T)%>%
#set first col and header row
column_spec(1, bold = T, border_right = F, background = "white", width = "20em") %>%
row_spec(0,align = "c")%>%
#conditional formatting
column_spec(2,color = "white", background =cond_secnum(tab.numsec[2]))%>% #popover = paste("am:", tab.numsec$`2018 Fall`)
column_spec(3,color = "white", background =cond_secnum(tab.numsec[3]))%>%
column_spec(4,color = "white", background =cond_secnum(tab.numsec[4]))%>%
column_spec(5,color = "white", background =cond_secnum(tab.numsec[5]))%>%
column_spec(6,color = "white", background =cond_secnum(tab.numsec[6]))%>%
column_spec(7,color = "white", background =cond_secnum(tab.numsec[7]))%>%
column_spec(8,color = "white", background =cond_secnum(tab.numsec[8]))%>%
column_spec(9,color = "white", background =cond_secnum(tab.numsec[9]))#%>%footnote("")#ncol(tab.numsec)
#inctead of kbl; can use gt::gt()%>%tab_header(title = "How many core course sections are offered each term?")

#interactive table
interactive_tab.numsec<-datatable(data = tab.numsec,class = "compact",#display
...
```{r viztable shown, include=TRUE}
static_tab.numsec
interactive_tab.numsec
```

```{r vizplot, include=TRUE}
sectiontally%>%group_by(term,CourseGroup)%>%summarise(section_n=n())%>%
