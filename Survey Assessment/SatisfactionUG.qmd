...
viz_hist.rate<-tibble(year=c(2017,2018,2019,2020,2021,2022),
       resprate=c(.3,.22,.35,.5,.36,nrow(ugsatis22.quan)/nrow(enroll22f)))%>%
  ...
  annotate("text",x=2022,y=.24,hjust=0.7,size=3,fontface="bold",label="(330 responses)")+
  labs(x="", y="")+ theme_lz()+theme(axis.text.x = element_text())
```

```{r tbl_demographyNs}
#basics in a for loop constructed
gpvar1.tbl<-ugsatis22.quan%>%
  group_by(ugsatis22.quan[,gpvar.names[1]])%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%
  mutate(section=gpvar.names[1])%>%rename(type=gpvar.names[1])%>%select(section,type,prt)
...
for (i in 1:length(gpvar.names)){
  gpvar.tbl_i<-ugsatis22.quan%>%
  group_by(ugsatis22.quan[,gpvar.names[i]])%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%
  mutate(section=gpvar.names[i])%>%rename(type=gpvar.names[i])%>%select(section,type,prt)
  i=i+1
  gpvar.tbl<-gpvar.tbl%>%rbind(gpvar.tbl_i)}

gpvar.tbl%>%filter(!section%in%c("status","ppid","demography.sex","curriculum","cohort_id","college_attend"),
    !is.na(type),!type%in%c("Not applicable","Prefer not to say","Prefer to self-describe (Please specify):"))
