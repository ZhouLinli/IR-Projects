...
#22fall
coh16<-coh16%>%mutate(stillenroll22fall=if_else(
  coh16$PC_ID %in% enroll22fall$`People Code Id`,"Yes","No"))
#number of still enrolled
coh16%>%count(stillenroll22fall)#zero still enroll in 2022 spring

#22spring
coh16<-coh16%>%mutate(stillenroll22spring=if_else(
  coh16$PC_ID %in% enroll22spring$`People Code Id`,"Yes","No"))
#number of still enrolled
coh16%>%count(stillenroll22spring)#two still enroll in 2022 spring

#report gender and eth
coh16%>%filter(stillenroll22spring=="Yes")%>%count(GENDER_CODE,ETHNICITY_REPORT_DESC)
```
...
```{r stillenroll}
#22spring
coh14%>%mutate(stillenroll22spring=if_else(
  coh14$PC_ID%in%enroll22spring$`People Code Id`,"Yes","No"
))%>%count(stillenroll22spring)#zero still enroll in 2022 spring
