...
```{r calcualte margin of error}
#formula is: margin of error = z score * sigma/ sqrt(n)
#moe for ho
sd(nsse18.s$`Higher-Order Learning`,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(nsse18.s$`Higher-Order Learning`),]))*1.96

sd(nsse18.s$`Reflective and Integrative Learning`,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(nsse18.s$`Reflective and Integrative Learning`),]))*1.96

sd(nsse18.s$`Learning Strategies`,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(nsse18.s$`Learning Strategies`),]))*1.96

sd(nsse18.s$`Quantitative Reasoning`,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(nsse18.s$`Quantitative Reasoning`),]))*1.96

sd(nsse18.s$`Collaborative Learning`,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(nsse18.s$`Collaborative Learning`),]))*1.96

sd(nsse18.s$`Discussions with Diverse Others`,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(nsse18.s$`Discussions with Diverse Others`),]))*1.96


moe<-function(x) {
  sd(x,na.rm=T)/sqrt(nrow(nsse18.s[!is.na(x),]))*1.96
}
