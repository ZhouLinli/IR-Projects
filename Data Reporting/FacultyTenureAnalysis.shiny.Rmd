...
# Trends for Individual Departments (with 2016-2021 data)
...
average.dep<-dep.wide%>%select(Department,Average,Change)%>%arrange(-Average)%>% ungroup%>%mutate(Rank=rank(-Average))#needs to ungroup before row_number or rank
...
change.avgrank<-merge(change.dep,average.dep)%>%select(Department,Change,Average,Rank)%>%arrange(-Change)
...
change.avgrank%>%formattable(list(
  ...
  Rank= formatter("span",
                  style=x~style(color=ifelse(
                    x<10,"darkgreen",
                    ifelse(x>20,"red","grey")
                  )))
))
```
