...
satis.life.figure<-prtdf%>%filter(item%in%c("satis.activity", "satis.foodav", "satis.residstaff", "satis.residhall"),year>2019)%>%
  ...
  geom_line(aes(y = prt, x=year, group=item, 
            ...
            stat="identity", show.legend = F)+#
  scale_color_manual(name="",values=c("#6C6F70","#FFCB4F","#6C6F70","#6C6F70"))+#lightyellow#FFCB4F
  scale_linetype_manual(name="",values=c("dashed","solid","dashed","dashed"))
```
...
service.use.figure<-prtdf%>%filter(item%in%c("service.aac.use", "service.acadvisor.use", "service.classtech.use", "service.techhelp.use","service.counseling.use", "service.health.use", "service.shuttle.use" ),year>2019)%>%
  ...
  theme_lz()+theme(legend.position = "right",legend.direction = "vertical")+
  geom_text(aes(label= ifelse(year==2022,as.character(round(prt,2)),"")),
            size=2.8,fontface="bold",position = position_dodge(0.9))+
  geom_text(aes(label= ifelse(year!=2022,as.character(round(prt,2)),"")),
            size=2.8,color="#6C6F70",fontface="bold",position = position_dodge(0.9))+
  ...
  geom_line(aes(y = prt, x=year, group=item, 
            ...
            stat="identity", show.legend = F)+#
  scale_color_manual(name="",values=c("#6C6F70","black","#6C6F70","#6C6F70","#6C6F70","#6C6F70","#6C6F70"))+
  scale_linetype_manual(name="",values=c("dashed","solid","dashed","dashed","dashed","dashed","dashed"))
...
```{r service.satis figure, include=TRUE}
