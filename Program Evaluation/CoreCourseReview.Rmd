...
corepath<-corepath%>%group_by(people_code_id,CourseGroup)%>%count(term)%>%
  select(-n)%>%pivot_wider(names_from = CourseGroup,values_from = term)

corepath%>%View()

#check for n-per-key unique

#cannot save with vector in one cell: write_xlsx(corepath,"/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Data and Analyses/Program Review/2022 Core Curriculum/path_2018cohort.xlsx")
