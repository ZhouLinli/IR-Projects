---
title: "Science and Inquiry Knowledge and Perspectives (SIKP) Courses"
subtitle: "Review of Major Distribution"
format: html
editor: visual
---
...
#grades of core courses: contains "curriculum", "GPA"/C("grade") for each "coursegroup" and "coursecode" by "term"(18fall-22spring)
...
#check: stucrs_1822allcrs%>%count(term); stucrs_1822allcrs%>%count(coursecode); stucrs_1822allcrs%>%count(curriculum)%>%View()
```
...
viz.crscnt<-tab.crscnt%>%arrange(coursecode)%>%
kbl(align = "c",booktabs = T,label="tab.label", caption = "SIKP Course Enrollment by Semester", col.names =c("",str_wrap(names(tab.crscnt)[2:ncol(tab.crscnt)],width = 5))) %>%
...
```{r show.crscnt, include=TRUE}
viz.crscnt
```

# Distributions of the Major of Enrolled Students in SIKP Courses

Plots below summarize distributions of students' major for each SIKP course groups. Plots are created using aggregated data across academic year 2018-2022. All plots below excluded majors with less than 2% percentage.

```{r request notes}
#count major (with toal row&col) for each coursecode across 18-22
stucrs.si%>%group_by(coursegrp,curriculum)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%select(-n)%>%
  pivot_wider(names_from="coursegrp",values_from="prt",values_fill =0)%>%View()#too complicated, must 1) organize by course category 2)merge all terms into one
...
viz_bio.major<-stucrs.si%>%filter(grepl("^BIO",coursecode))%>%#check: %>%count(coursecode)
  group_by(curriculum)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%arrange(-prt)%>%#check:View()
  select(-n)%>%filter(prt>.02)%>%

ggplot(aes(y=reorder(curriculum,prt), x = prt)) +  #y=factor(itemvar/yvar, levels = setyorder$yorder, ordered = TRUE), 
...
```{r longdisplay.viz_major,fig.height=6,include=TRUE}
...
```{r middisplay.viz_major,fig.height=4,include=TRUE}
gridExtra::grid.arrange(viz_bio.major,#12
                        viz_chem.major,#12
                        nrow=1, widths=c(1,1))
```

```{r shortdisplay.viz_major,fig.height=2.8,include=TRUE}
gridExtra::grid.arrange(viz_cj.major,#3
                        viz_phys.major, #9
                        nrow=1, widths=c(1,1))
```

```{r explaincurriculumacyname}
#stucrs_1822allcrs%>%count(curriculum,Major)%>%filter(!is.na(Major))%>%View()
```
