---
title: "Academic Achievement Center (AAC) Courses Enrollment and Grades Review (2018 Fall - 2022 Spring)"
...
static_tab.aacenroll<-column_spec(
  ...
  column=i, width = "3em",background =cf_color(tab.aacenroll[i],a=70,b=60))
i=i+1
...
enrollment range from `r min(n_aacenroll$n)` to `r max(n_aacenroll$n)`.
An average enrollment for AAC103 is
...
Average pass rate for AAC courses are above
`r mean(tab.aacpass_avg$Average)%>%percent(digit=0)` across the past 5
academic years.
...
column_spec(4, width = "4em", background = if_else(tab.semgpa_pass[4]>.5,color_grey,if_else(tab.semgpa_pass[4]>.4,color_greylight,"white")))
...
Students with higher than 2.0 semester GPA usually have a
`r min(tab.semgpa_pass$Difference)` to
`r max(tab.semgpa_pass$Difference)` higher AAC 103 pass rate than
students with lower than 2.0 semester GPA.
...
students in AAC 102 course have a higher than 2.0 semester GPA in the
past 5 years. The average percentage of higher than 2.0 semester GPA for
AAC 103 course is
...
Among students who passed AAC 103, an average of
`r tab.semGPA_passfail_avg[tab.semGPA_passfail_avg$passfail=="Pass","Average"][[1]][1]`
students have a higher than 2.0 semester GPA. In contrast, only an
average of
...
In analysis, we found `r N[4]` students took AAC 103 4 times and
`r N[5]` of them took the course 5 times. Due to the small sample size,
we exclude those students from pass rate reviewing. The following tables
reported students who took AAC 103 for the 1st, 2nd, and 3rd time.

First time takers has the highest pass rate of
`r tab.retakepass[1,"Pass rate"]%>%percent(digit=0)`, while the pass rate decreased to
`r tab.retakepass[2,"Pass rate"]%>%percent(digit=0)` and `r tab.retakepass[3,"Pass rate"]%>%percent(digit=0)`
when students retaking the course for the second and third time. In
other words, AAC 103's pass rate did not increase for along with
students times of retaking the course.
...
from `r min(tab.retake_semGPA[,"Semester GPA >2"])%>%percent(digit=0)`
to `r max(tab.retake_semGPA[,"Semester GPA >2"])%>%percent(digit=0)`,
with the second time being the highest percentage of higher than 2.0
GPA.
...
-   Among the past 5 years (2018 Fall to 2022 Spring), AAC 103 course
    has an average enrollment of
    `r n_aacenroll[n_aacenroll$coursecode=="AAC103","Average"][[1]][1]`
    students.
    
-   Average pass rate for AAC 103 course is
...
-   An average of
    `r tab.semGPA_avg[tab.semGPA_avg$coursecode=="AAC103","Average"][[1]][1]`
    students in AAC 103 course have a higher than 2.0 semester GPA.
    
-   Among those who pass AAC 103,
    `r tab.semGPA_passfail_avg[tab.semGPA_passfail_avg$passfail=="Pass","Average"][[1]][1]`
    ...
    103,
    `r tab.semGPA_passfail_avg[tab.semGPA_passfail_avg$passfail=="Fail","Average"][[1]][1]`
    still have a higher than 2.0 semester GPA (without succeeding in AAC
    103 course).
    
-   Taking AAC 103 the second and third times does not improve pass
    rate. First time takers has the highest pass rate of
    `r tab.retakepass[1,"Pass rate"]%>%percent(digit=0)`, while the pass rate decreased to
    `r tab.retakepass[2,"Pass rate"]%>%percent(digit=0)` and
    `r tab.retakepass[3,"Pass rate"]%>%percent(digit=0)` when students retaking the course
    for the second and third time.
    
-   Percentage of obtaining higher than 2.0 semester GPA are similar for
    students who take AAC 103 for the first and second time, which are
