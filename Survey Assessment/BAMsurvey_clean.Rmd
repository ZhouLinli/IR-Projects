...
bam.long%>%count(act21,act21_value)%>%
  ...
  select(act21_value,prt)%>%filter(!is.na(act21_value))
```
...
```{r}
bam%>%filter(!is.na(itemvar))%>%group_by(fillvar,itemvar)%>%summarise(cnt=n())%>%mutate(prt=formattable::percent(cnt/sum(cnt),digits=0))%>%
  
ggplot(aes(x = prt, y=reorder(itemvar,(-prt), fill=fillvar)) +  
  geom_bar(stat="identity",position = position_dodge(),width=0.9)+
  scale_fill_manual(values = c("grey","#69b3e7"))+

  scale_y_discrete(labels=c("Commuters","Residents"))+#  scale_x_continuous(labels = scales::percent)+
