...
```{r html-`Finance`}
#read html and parse it into R readable contents
pg<-read_html("https://www.lasell.edu/academics/academic-catalog-21-22/undergraduate-catalog-21-22/programs-of-study-/finance-21-22.html#tab-2")
#look at it (a list object that contains the tree-like structure)
pg
#search using css class as nodes
pg %>% html_nodes(".mp-modal")%>%html_text()#we must add a . before the class name
...
```{r keep core course and recode using crs.df}
#investigate
dat.major%>%group_by(`Finance`)%>%count()
#recode using case_when (new var= old var name)
dat.major<-
  dat.major%>%mutate(`Finance`=case_when(
    `Finance`=="C"~"Core",
    Section %in% crs.df$Section[1:20] ~"Required",
    Section %in% crs.df$Section[21:35] ~"Elective"))#%>%group_by(`Finance`)%>%count()#check directly after mutate, before assign it to df
```