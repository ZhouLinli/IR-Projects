...
::: notes
- White tenured faculty proportion stable in range 42-48%
- Asian and Internal 
::: 

```{r facet two race bulk group,fig.width=15}
#race count
nrace<-data%>% filter(`Race/Ethnicity`!=c("Unreported",NA))%>%group_by(`Race/Ethnicity`)%>%count()%>%arrange(-n)%>%mutate(race=factor(`Race/Ethnicity`))

#create two bulk group and pivot table
racepvt<-data%>%filter(`Race/Ethnicity`!=c("Unreported",NA))%>%mutate(RaceGroup=case_when(
  `Race/Ethnicity` %in% nrace$`Race/Ethnicity`[1:3] ~ "Larger Racial Group",
  `Race/Ethnicity` %in% nrace$`Race/Ethnicity`[4:6] ~ "Smaller Racial Group",))%>%
#pivot table  
...
#max/min data label
#max.min<-racepvt%>%select(`Race/Ethnicity`,prt)%>%group_by(`Race/Ethnicity`)%>%summarise(max=max(prt),min=min(prt))
  
#plot
racepvt%>%ggplot(aes(color=factor(`Race/Ethnicity`),x=factor(Year),y=prt,group=factor(`Race/Ethnicity`)))+geom_line()+
  scale_color_manual(name="",limits=nrace$race, values = c(mycolors[1],mycolors[2],mycolors[4],mycolors[6],mycolors[7],"green"),labels=c("White","Asian","International","Hispanic","Black","Muti-races"))+  
  
  facet_wrap(~RaceGroup,labeller=labeller(RaceGroup= c(`Larger Racial Group`="Larger Racial Groups (n>80)", `Smaller Racial Group`="Smaller Racial Groups (n<80)") ) )+
  
#geom_text_repel(aes(label=prt),fontface="bold",size=5)+
#themes
scale_y_continuous(breaks = seq(.2,1,by=.2),labels = scales::percent)+
theme_lz_ppt()+theme(axis.text.y=element_text(size=12))+theme(panel.grid.major.y = element_line(color = "grey95"))+#theme(legend.position = "right", legend.direction = "vertical")+
