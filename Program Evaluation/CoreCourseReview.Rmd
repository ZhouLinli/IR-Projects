...
#save: writexl::write_xlsx(aac.crs,"/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Data and Analyses/Program Review/2022 AAC/StudentInfoCourse_18FA-22SP_aac.xlsx")
#openxlsx::write.xlsx(aac.crs, file = "/Users/linlizhou/Downloads/AACcoursereview11.10.xlsx", sheetName="rawcrs", append=TRUE)
#library(openxlsx)
wb<-loadWorkbook("/Users/linlizhou/Downloads/AACcoursereview11.10.xlsx")
addWorksheet(wb,"rawcrs")
writeData(wb,"rawcrs", x = aac.crs)
#worksheetOrder(OUT) <- c(2,1)
saveWorkbook(wb,"/Users/linlizhou/Downloads/AACcoursereview11.18.xlsx",overwrite = TRUE)
```


```{r unduplicated enrollment}
#exclude students who dropped the class
aac.crs%>%filter(Add_Drop!="D")%>%
group_by(term,coursecode)%>%summarise(n=n_distinct(people_code_id))%>%
#%>%select(n)%>%colSums(n)#10549 ppid adding separate terms together
  pivot_wider(names_from = term,values_from = n)%>%View()
```
...
aac.crs<-aac.crs%>%mutate(passfail=case_when(Final_Grade=="P"~"Pass",
                                    Final_Grade%in%c("F")~"Fail"))#check:%>%count(passfail)
