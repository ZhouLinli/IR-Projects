...
```{r duplicated term credit}
#ppid took two level courses in the same term 
...
tot.crd<-ug.gd%>%group_by(ppid)%>%summarise(tot.credit=sum(`Term Credits`))
#check: tot.crd%>%group_by(ppid)%>%summarise(cnt=n())%>%arrange(desc(n))#each id only have one tot.credit value
...
ipeds<-ug.gd%>%select(-`Cum Credits`,-Program,-`gov id`,-`Birth Date`,-term,-`Term Credits`)%>%
  unique()%>%arrange(ppid)#2058
