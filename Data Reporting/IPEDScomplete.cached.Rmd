...
#merge
grad19.20t<-join_all(list(grad19sum,grad20spr,grad19f),type="full",match="first")#548 observations
#rename for selected cols
ipeds.complete20f<-grad19.20t%>%select(people_code_id,program,degree,major,graduation_date,Reporting_Ethnicity,gender)%>%
  dplyr::rename(PCID=people_code_id,level=program,gradd=graduation_date,race=Reporting_Ethnicity)
#remove
rm(grad19.20t,grad19f,grad19sum,grad20spr)

#####reformat date#####
class(ipeds.complete20f$gradd)#it's character
#convert the mdy formated character to date format
ipeds.complete20f$gradd<-mdy_hms(ipeds.complete20f$gradd)
class(ipeds.complete20f$gradd)#now it's date
...
#merge
grad18.19t<-join_all(list(grad18sum,grad19spr,grad18f),type="full",match="first")#548 observations
#rename for selected cols
ipeds.complete19f<-grad18.19t%>%select(people_code_id,program,degree,major,graduation_date,Reporting_Ethnicity,gender)%>%
  dplyr::rename(PCID=people_code_id,level=program,gradd=graduation_date,race=Reporting_Ethnicity)
#remove
rm(grad18.19t,grad18f,grad18sum,grad19spr)

#####reformat date#####
class(ipeds.complete19f$gradd)#it's character
#convert the mdy formated character to date format
ipeds.complete19f$gradd<-mdy_hms(ipeds.complete19f$gradd)
class(ipeds.complete19f$gradd)#now it's date
...
#merge
grad17.18t<-join_all(list(grad17sum,grad18spr,grad17f),type="full",match="first")#548 observations
#rename for selected cols
ipeds.complete18f<-grad17.18t%>%select(people_code_id,program,degree,major,graduation_date,Reporting_Ethnicity,gender)%>%
  dplyr::rename(PCID=people_code_id,level=program,gradd=graduation_date,race=Reporting_Ethnicity)
#remove
rm(grad17.18t,grad18f,grad17sum,grad18spr)

#####reformat date#####
class(ipeds.complete18f$gradd)#it's character
#convert the mdy formated character to date format
ipeds.complete18f$gradd<-mdy_hms(ipeds.complete18f$gradd)
class(ipeds.complete18f$gradd)#now it's date
...
#merge
grad16.17t<-join_all(list(grad16sum,grad17spr,grad16f),type="full",match="first")#548 observations
#rename for selected cols
ipeds.complete17f<-grad16.17t%>%select(people_code_id,program,degree,major,graduation_date,Reporting_Ethnicity,gender)%>%
  dplyr::rename(PCID=people_code_id,level=program,gradd=graduation_date,race=Reporting_Ethnicity)
#remove
rm(grad16.17t,grad17f,grad16sum,grad17spr)

#####reformat date#####
class(ipeds.complete17f$gradd)#it's character
#convert the mdy formated character to date format
ipeds.complete17f$gradd<-mdy_hms(ipeds.complete17f$gradd)
class(ipeds.complete17f$gradd)#now it's date
```
