...
```{r course title}
pg%>%html_nodes(xpath="/html/body/div[1]/main/div/div/div[1]/div/div/div/div[2]/div[1]/table/tbody/tr[2]/td[2]") %>% html_text()
pg%>%html_nodes(xpath="/html/body/div[1]/main/div/div/div[1]/div/div/div/div[2]/div[1]/table/tbody/tr[3]/td[2]") %>% html_text()

pg%>%html_node(".td")%>%html_text()
```
...
```{r html-`Fashion Media and Marketing`}
#read html and parse it into R readable contents
pg<-read_html("https://www.lasell.edu/academics/academic-catalog-21-22/undergraduate-catalog-21-22/programs-of-study-/biology-21-22.html#tab-2")
#look at it (a list object that contains the tree-like structure)
pg
#search using css class as nodes
pg %>% html_nodes(".mp-modal")%>%html_text()#we must add a . before the class name
...
dat.major<-
  dat.major%>%mutate(`Fashion Media and Marketing`=case_when(
   # `Fashion Media and Marketing`=="C"~"Core", #hide for not-existing major
    Section %in% crs.df$Section[1:length(crs.df$Section)] ~"Required"))#%>%group_by(`Fashion Media and Marketing`)%>%count()#check directly after mutate, before assign it to df
...
# Clean Column Names (select only col/major in the web major list)
...
# Course Title for Newly Added Course IDs

```{r}
#investigate
dat.48major[is.na(dat.48major$`Course Title`),1]#93 course does not have a title

###########load section-title list
#load relevant data from Section Tally Report (MyLasell Report-Section Tally in report manager)
##csv files does not have formatted cols to read
sec.df<-plyr::join_all(list(
  read_excel("/Users/linlizhou/Downloads/21fsection.xlsx"),
  read_excel("/Users/linlizhou/Downloads/21sumsection.xlsx"),
  read_excel("/Users/linlizhou/Downloads/21wsection.xlsx"),
  read_excel("/Users/linlizhou/Downloads/22spsection.xlsx")
  ),
  type="full")
#select 2 cols, remove duplicated or nas
sec.df<-sec.df%>%select(`...2`,`...6`)%>%unique()%>%drop_na()
#rename to match to be merged dataset
names(sec.df)=c("Section","Course Title")
#save

write_xlsx(sec.df,"/Users/linlizhou/Documents/LASELL/data/enrollment/sectally21.22.xlsx")
```

```{r merge to fill}
dat.48major2<-dat.48major%>%mutate(`Course Title`=case_when(
  is.na(dat.48major$`Course Title`)==FALSE ~ dat.48major$`Course Title`,
  dat.48major[is.na(dat.48major$`Course Title`),1] %in% sec.df$Section ==TRUE ~ sec.df$`Course Title`
))

#check
sec.df[,]
  #93 course does not have a title
```
