...
ipeds.enroll.ages<-ug.gd%>%select(age,ppid)%>%unique()
age.y<-ipeds.enroll%>%filter(!is.na(age))%>%select(ppid,age)#is not na
age.n<-ipeds.enroll%>%filter(! ppid %in% age.y$ppid)%>%select(ppid,age)#is 


#remove name duplication
ipeds.enroll.names<-ug.gd%>%select(Name,ppid)%>%unique()
#merge the first name only
ipeds.enroll.t<-plyr::join_all(list(ipeds.enroll, ipeds.enroll.names), type = "left", match = "first")%>%unique()
