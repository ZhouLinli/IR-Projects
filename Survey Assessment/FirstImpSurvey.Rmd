...
allstu22<-allstu%>%
  ...
  select(`People Code Id`, Name,`Lasell email`,`Transfer YN`,`Res/Comm`,Gender,Ethnicity)#select all usable vars
...
fis.s<-fis%>%select(-(`Response ID`:Source),-(`Invite Custom Field 2`:Gender.x))%>%#removed 54 cols
...
```{r making fig for respondents}
fis.s%>%ggplot(aes(RCstatus)) + geom_bar();    ggplot(aes(x,fill)) + geom_bar()

ggplot(aes(x,y)) + geom_bar(stat=""identity"")  #By default, geom_bar() will plot counts of discrete values. geom_bar(stat=""identity"") will plot a single value on the y-axis.

#stack vs side-by-side
ggplot(data, aes(x = quarter, y = profit, fill = gender)) +geom_col/bar(stat=""identity"") 是stakced +scale_fill_manual(...)
ggplot(data, aes(x = quarter, y = profit, fill = gender)) +geom_col/bar(position = position_dodge(stat=""identity""))就是毗邻的+scale_fill_manual(...)

#柱状图加线图
ggplot(aes(x,y,fill)) +geom_bar(stat=""identity"",position=position_dodge(),width=.8) +geom_text()
  geom_line(aes(x,samey),color="".."")
```
