...
```{r response rate figure}
#response rate
response_rate<-fulldf%>%count(RespondentOverall)%>%
ggplot(aes(x = n, y=RespondentOverall, fill=RespondentOverall)) +
  geom_col()+
  labs(title=str_wrap(paste0(response_rate," Response Rate"),35),#80 is a full line length
       x="Number of Students",y="",fill="", 
       subtitle=paste0(num.response, " out of ", num.overall, " answered the First Impression Survey"), 
       caption="Data collection: Sep 20-30, 2022")+ #shows bottom right
  theme_void()+theme(legend.position="none")+
  scale_fill_manual(values = c("#6C6F70","#69b3e7"))+#fill legend labels=c(""),name="")
  #scale_y_discrete(labels=c("Survey Respondents","All First Year Students"))+
  geom_text(aes(label=paste0(RespondentOverall,": ",n)),color="white",position = position_dodge(0.9),hjust=1.5)
```


```{r respondent backgrounds figure}
#background info: gender & ethnicity, FT/PT & Transfer, RC
#RC
fulldf%>%group_by(RespondentOverall,RCstatus)%>%summarise(cnt=n())%>%
  mutate(prt=formattable::percent(cnt/sum(cnt),digits=0))%>%
  
ggplot(aes(x = prt, fill=RespondentOverall, y=RCstatus)) +
  geom_bar(stat="identity",position = position_dodge())+
  labs(title=str_wrap("Representative Resident vs Commuters Composition",35),#80 is a full line length
       x="",y="",fill="", 
       subtitle=paste0("6% Residents and 94% Commuters"), 
       caption="")+ #shows bottom right
  theme_void()+theme(legend.position="bottom",
                     axis.text.y = element_text())+
    scale_y_discrete(labels=c("Commuters","Residents"))+
  geom_text(aes(label=prt),color="white",position = position_dodge(0.9),hjust=1)+#, " ", RCstatus," among ",RespondentOverall
  scale_fill_manual(values = c("#6C6F70","#69b3e7"))#fill legend labels=c(""),name="")
