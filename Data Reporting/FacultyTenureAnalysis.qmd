...
#pivot table
data%>% filter(`Race/Ethnicity`%in% nrace$`Race/Ethnicity`[1:3])%>% group_by(Year, `Race/Ethnicity`,`Tenure Status`)%>%#2016 male as a group -> proportions of (non) tenure for all male professors in 2016
summarise(cnt=n_distinct(ID))%>%mutate(prt=percent(cnt/sum(cnt),digits=0))%>%filter(`Tenure Status`=="Tenure Line")%>%
#plot
...
theme_lz_ppt()+theme(axis.text.y=element_blank())+
labs(title="Tenured Faculty in Large Racial Groups (n>80)",x="",y="")
```
...
```{r tenured trend by race bottom 3}
#pivot table
data%>% filter(`Race/Ethnicity`%in% nrace$`Race/Ethnicity`[4:6])%>% group_by(Year, `Race/Ethnicity`,`Tenure Status`)%>%#2016 male as a group -> proportions of (non) tenure for all male professors in 2016
summarise(cnt=n_distinct(ID))%>%mutate(prt=percent(cnt/sum(cnt),digits=0))%>%filter(`Tenure Status`=="Tenure Line")%>%
#plot
ggplot(aes(color=factor(`Race/Ethnicity`),x=factor(Year),y=prt,group=factor(`Race/Ethnicity`)))+geom_line()+
...
scale_y_continuous(breaks = seq(.3,.5,by=.1),labels = scales::percent)+
theme_lz_ppt()+theme(axis.text.y=element_blank())+
labs(title="Tenured Faculty in Small Racial Groups (n<80)",x="",y="")
```
...
scale_color_discrete(name="")+
theme_lz_ppt()+theme(axis.text.y=element_text(size=12))+theme(panel.grid.major.y = element_line(color = "grey"))+
