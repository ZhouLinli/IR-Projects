...
```{r load finaid16fall data}
finaid16fall<-read_excel("/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Fin Aid Sharing/Fall 2016 freeze file.xlsx")
#check: 
#check no duplication: finaid16fall%>%count(`Student Ssn`)%>%count(n)
```

```{r explore finaid_colums}
##find relevant vars
#glimpse(finaid16fall)
#names(finaid16fall)#find index
#lapply(finaid16fall[,48:54], unique)#use index to view unique values
##relationship between pell related grants: finaid16fall%>%count(`Pell Award Calc`,`All Federal Grant awarded`)
...
```{r remove-cannot find ssn of dgskFTft students}
...
```{r verify dgskFTft.dmt = dgskFTft.reg}
...
```{r mutate dgskFTft vars to finaid}
...
#check: glimpse(finaid16fall$`Student Ssn`);glimpse(finaid16fall)
#check: glimpse(dmt_dgskFTft.vars$SSN);glimpse(dmt_dgskFTft.vars)

#mutate to finaid
finaid16fall.merge_dgskFTft<-left_join(finaid16fall,dmt_dgskFTft.vars,by=c(`Student Ssn`="SSN"))%>%#1842
...
#approach2: finaid16fall%>%filter(`Student Ssn` %in% as.numeric(dgskFTft.dmt$SSN))%>%str()#1436 dgskFTft students
```

```{r save finaid16fall.merge_dgskFTft}
wb<-createWorkbook()
#add new sheet "finaid16fall"
addWorksheet(wb,"finaid16fall")
#assign classsize as clean sheet
writeData(wb,"finaid16fall", x = finaid16fall)
...
#assign classsize as clean sheet
writeData(wb,"merged.dgskFTft", x = finaid16fall.merge_dgskFTft)
...
```{r load retention file}
retentionfile<-read_excel("/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Data and Analyses/Retention/2022-2023/Interactive UG Retention Analysis.xlsx",sheet = "Data")

#check: glimpse(retentionfile)
#relevant vars: 
#cohort ID==2016
#Ret Sem any 2-8 is "Y" ("complete in 4 years or less)
#Ret Sem 8 is NA, but 9 or 10 is "Y" ("complete  more than 4 years but in 5 years or less)
#Ret Sem 10 is NA, but 11 or 12 is "Y" ("complete  more than 5 years but in 6 years or less)
#Ret Sem any 2-12 is "Y" ("complete in 6 years or less)

#select releavnt 
```

```{r retention merge finaid16}
finaid16fall<-read_excel("/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Fin Aid Sharing/Fall 2016 freeze file.xlsx")
#relevant vars
#confirm if all cohort 2016 students are in this file 
#confirm if their fall finaid status is the status, no more students is receiving in later semesters
#
```
