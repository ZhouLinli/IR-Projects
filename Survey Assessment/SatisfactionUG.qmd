...
tbl_samvspop<-tbl_samvspop%>%arrange(section,type,-Sample)%>%
  ...
  mutate(Diff=Sample-Population)%>%select(type,Sample,Population,Diff)
```
...
cut1=.5;cut2=.3;cut3=.1;cut4=.03
for (i in 2:ncol(tbl_samvspop)){viz.tbl_samvspop<-column_spec(kable_input=viz.tbl_samvspop,column=i, width = "3em",color="white", background =cf_color_blue(tbl_samvspop[i],a=cut1,b=cut2,c=cut3,d=cut4)); i=i+1;viz.tbl_samvspop}
