...
```{r merge degseek.full}
#merge
degseek.full<-left_join(degseek,dmtfa22.state,by=c("People Code Id"="PC_ID"))%>%#check names(degseek.full)
...
#check duplication: degseek.full%>%group_by(ppid)%>%count()%>%group_by(n)%>%count()#1236degseek.full students

#see what are the values: degseek.full%>%select(ADMIT_STATE,ADMIT_COUNTRY,ADMIT_REGION)%>%lapply(unique) #NA in state/country/region  
#see NA: degseek.full%>%filter(is.na(ADMIT_COUNTRY)|is.na(ADMIT_REGION)|is.na(ADMIT_STATE))%>%select(ADMIT_STATE,ADMIT_COUNTRY,ADMIT_REGION)
```

```{r recode admit state_region_country}
#use one to fill each other
##recode country for counvience: degseek.full%>%count(ADMIT_COUNTRY)
degseek.full<-degseek.full%>%mutate(ADMIT_COUNTRY=
              if_else(is.na(ADMIT_COUNTRY),ADMIT_COUNTRY,
                 if_else(ADMIT_COUNTRY=="United States","US","intl")))#check:%>%count(ADMIT_COUNTRY)

##recode region for counvience: degseek.full%>%count(ADMIT_REGION)
degseek.full<-degseek.full%>%mutate(ADMIT_REGION=
...
##recode state for convinience: degseek.full%>%count(ADMIT_STATE)%>%View()
degseek.full<-degseek.full%>%mutate(ADMIT_STATE=
              if_else(is.na(ADMIT_STATE),ADMIT_STATE,
                 if_else(ADMIT_STATE%in%c("MA","Massachusetts"),"MA",
                      if_else(ADMIT_STATE%in%c("AL",  "AK",  "AZ",  "AR",  "CA",  "CO",  "CT",  "DE",  "FL",  "GA",  "HI",  "ID",  "IL",  "IN",  "IA",  "KS",  "KY",  "LA",  "ME",  "MD",  "MI",  "MN",  "MS",  "MO",  "MT",  "NE",  "NV",  "NH",  "NJ",  "NM",  "NY",  "NC",  "ND",  "OH",  "OK",  "OR",  "PA",  "RI",  "SC",  "SD",  "TN",  "TX",  "UT",  "VT",  "VA",  "WA",  "WV",  "WI",  "WY","Maine"),"other states","intl"))))   #check:%>%count(ADMIT_STATE)# AE (United Arab Emirates) is coded as intl
```

```{r fill region NA}
#check: degseek.full%>%select(ADMIT_STATE,ADMIT_COUNTRY,ADMIT_REGION)%>%lapply(unique)
#see NA: degseek.full%>%filter(is.na(ADMIT_COUNTRY)|is.na(ADMIT_REGION)|is.na(ADMIT_STATE))%>%select(ADMIT_STATE,ADMIT_COUNTRY,ADMIT_REGION)

#use region to fill country: degseek.full%>%count(ADMIT_COUNTRY); degseek.full%>%count(ADMIT_REGION)
#degseek.full<-degseek.full%>%mutate(ADMIT_COUNTRY=
#              if_else(!is.na(ADMIT_COUNTRY),ADMIT_COUNTRY,
...
#use region to fill state: degseek.full%>%count(ADMIT_REGION); degseek.full%>%count(ADMIT_STATE)
degseek.full<-degseek.full%>%mutate(ADMIT_STATE=
              if_else(!is.na(ADMIT_STATE),ADMIT_STATE,
...
#use state to fill region: degseek.full%>%count(ADMIT_REGION); degseek.full%>%count(ADMIT_STATE)
degseek.full<-degseek.full%>%mutate(ADMIT_REGION=
              if_else(!is.na(ADMIT_REGION),ADMIT_REGION,
...
degseek.full<-degseek.full%>%mutate(ADMIT_REGION=
              if_else(!is.na(ADMIT_REGION),ADMIT_REGION,
                 if_else(ADMIT_COUNTRY=="intl","intl",ADMIT_REGION)))%>%count(ADMIT_REGION)#no change


#see NA: degseek.full%>%filter(is.na(ADMIT_REGION)|is.na(ADMIT_STATE))%>%select(ADMIT_STATE,ADMIT_COUNTRY,ADMIT_REGION)%>%View()#34 NA cross all admit cols
```
...
addWorksheet(wb,"clean")
#assign classsize as clean sheet
writeData(wb,"clean", x = degseek.full)
...
#save all in a new file
saveWorkbook(wb,"/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/Common Data Set/2022-2023/datafiles/Fall 2022 UG Backup Data Report_cleanCDS-F.xlsx",overwrite = TRUE)
```
...
ipeds.complete<-ipeds.complete%>%mutate(degree_awarded=
          case_when(Degree=="CER"~"certificates",
                    Degree=="AA"~"associate",
                    Degree%in%c("BA","BS")~"bachelor",
                    Degree%in%master.tbl$Degree~"master"))#check:%>%count(degree_awarded,Degree)
```
