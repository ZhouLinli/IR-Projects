...
knitr::opts_chunk$set(echo = FALSE, include = TRUE, warning=FALSE, message=FALSE, fig.align = "center") #display code along with its results
...
plot.title = element_text(family = font, size = 14, face = 'bold',hjust = 0.5, vjust = 2),

axis.title = element_text(family = font, size = 12),

axis.text = element_text(family = font, size = 12),

axis.text.x = element_text(family = font, size = 12, margin = margin(t=5,b=10)),
...
strip.text = element_text(family = font, size = 12, margin = margin(t=5,b=10)),#move up is + 1
...
geom_text(aes(label=prt),position=position_dodge(.9),vjust=1.5,color="white",fontface="bold", size= 9 )+theme_lz()+
...
ggplot(aes(fill=factor(`Tenure Status`),y=factor(`Race/Ethnicity`),x=prt))+
  ...
  geom_text(aes(label=prt),position=position_dodge(.9),hjust=1,color="white",fontface="bold", size = 9)+theme_lz()+
...
data%>%filter(Department %in% ndep[ndep$cnt>20,1]$Department ) %>%
  ...
  theme_lz()+geom_text(aes(label=prt),position = position_stack(),hjust=1,color="white", size = 9)+
