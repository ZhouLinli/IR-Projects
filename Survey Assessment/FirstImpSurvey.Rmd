...
```{r match ppid from allstu22 for fis.original}
...
#merge
fis.original<-left_join(fis.original,allstu22,by="email.short")#all fis.original with a valid email will have a ppid and names now
#check: sum(is.na(fis.original$`People Code Id`));sum(is.na(fis.original$Name))#only 16 do not find a matched ppid/name
```
...
ResAll.eth<-fis.allstu%>%filter(Ethnicity!="Unknown")%>%
  ...
  theme_lz()+theme(axis.text.x = element_text(),
...
```{r display resp_rate and combined figure,include=TRUE,fig.height=4}
...
```{r display eth figure, include=TRUE,fig.height=4}
...
```{r display empfaculty fig,include=TRUE}
...
```{r distill one NUMERIC MEAN for aggregated vars and SAVE numdf}
...
```{r display numfigure, include=TRUE}
num.figure
```
...
ethdiff<-fis.num%>%group_by(Ethnicity)%>%summarise(svmn=mean(service.counseling,na.rm=TRUE))%>%
  ...
  labs(title=str_wrap("",40),x="",y="",
       caption="*Counseling service satisfaction is the only survey item significantly different by ethnicity")+
  scale_x_discrete(labels=c("Hispanic (n=32)",
                            "Black (n=20)",
                            "White (n=112)",
                            "Asian (n=6)",
                            "Multi-races (n=4)"))+
...
ethdiff2<-fis.num%>%group_by(Ethnicity)%>%summarise(
  ...
  scale_color_manual(values=c(mycolors[7],mycolors[3],mycolors[6]),name="",labels=c("AAC","Advisor","Counseling"))+
  ...
  facet_wrap(~factor(Ethnicity,levels = c("Two or more Races","Asian","White","Black or African American","Hispanic"),labels=c("Multi-races (n=4)","Asian (n=6)","White (n=112)","Black (n=20)","Hispanic (n=32)")),nrow=1, strip.position="bottom") 
...
## Counseling Service Satisfaction
...
Please contact ir@lasell.edu if you have any questions
