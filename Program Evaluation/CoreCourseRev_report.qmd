...
kable(label="tab.stutrans",#in-text reference
    ...
    col.names =c(names(tab.stutrans)[1:ncol(tab.stutrans)])) %>%#header names
#set overall style
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
#fill the page width-wise,#header stay when scroll
              full_width = F, position = "float_left",fixed_thead = T, font_size = 12)%>%
...
row_spec(0,align = "c",color="black")%>%
#set first column: 
  column_spec(1, bold = T, border_right = F, background = "white", width = "7em",color="black")%>%
#cond formatting using different rules
column_spec(2,color = "white",background =cf(tab.stutrans[2],a=1300,b=1200))%>%
column_spec(3,color = "white",background =cf(tab.stutrans[3],a=0.185,b=0.15))
```
...
mutate(across(`2018 FALL`:`2022 SPRING`, function(x) percent(x, digit=0)))  #assign NA to ".."
...
static_tab.pass<-column_spec(
  ...
  column=i,color = "white",background =cf(tab.pass[i],a=.94,b=.75))
i=i+1
static_tab.pass}
...
static_tab.withdraw<-column_spec(
  ...
  column=i,color = "white",background =cf(tab.withdraw[i],a=.10,b=.05))
i=i+1
static_tab.withdraw}
```
...
static_tab.sci<-column_spec(
  ...
  column=i,color = "black",background =cf(tab.sci[i],a=.94,b=.6,col2 = "white"))
i=i+1
static_tab.sci}
```
