...
sec.names<-names(ugsatis22.quan)%>%str_extract("[a-z]+\\.")%>%as.tibble()%>%filter(!value %in% c("demography.",NA))%>%unique()
...
#check same answers: lapply(ugsatis22.quan%>%select(starts_with(sec.names[1])), unique)
ugsatis22.overall<-ugsatis22.quan%>%
  select(all_of(gpvar.names),starts_with(sec.names[1]))%>%#check: str()
  pivot_longer(cols=starts_with(sec.names[1]),names_to=sec.names[1],values_to=paste0(sec.names[1],"value"))#check:%>%str()
...
ggplot(aes(y=factor(overall., levels = setyorder$yorder, ordered = TRUE),
  ...
  geom_text(aes(label=if_else(prt>0.03,as.character(percent(prt,digits = 0)),"")),#do not show anything less than error terms, to avoid overlapping labels as well
    ...
    labs(title="",x="",y="",fill="")+theme_lz()+theme(axis.text.x = element_text(),legend.position = "bottom")+coord_flip()
```

## Overall Satisfaction (2018-2022)
...
ugsatis22.overall%>%filter(!is.na(overall.value))%>%
  group_by(overall.,overall.value)%>%summarise(n=n())%>%mutate(prt=n/sum(n))%>%
...
#viz
  ...
  scale_y_discrete(labels=c("Experience","Service","Facility"))+#1st label at bottom
  ...
  facet_wrap(~overall.,nrow=1, strip.position="bottom") +
