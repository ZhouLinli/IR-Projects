...
theme(axis.text.y=element_blank())+theme(axis.title.x =element_blank())
```

# Trends by Faculty Characteristics
...
#plot
ggplot(aes(color=factor(`Tenure Status`),x=factor(Year),y=prt,group=factor(`Tenure Status`)))+geom_line()+
geom_text(aes(label=prt),fontface="bold",size=6.5)+facet_wrap(~Rank)+
#themes
scale_y_continuous(breaks = seq(.25,.75,by=.1),labels = scales::percent)+
scale_x_discrete(labels=c("16","17","18","19","20","21"))+
scale_color_discrete(labels=c("Non-tenure","Tenure"),name="")+
theme_lz_ppt()+theme(axis.text.y=element_blank())+theme(axis.title.x =element_blank())+
labs(title="Tenure Status Trend by Faculty Rank",x="",y="")
```
