...
```{r report_responses, include = TRUE}
...
plot.act21<-bam.long%>%count(act21,act21_value)%>%
  ...
  scale_y_discrete(labels=c(str_wrap("Book: What Happened to You?",width = 13),
                            str_wrap("DEI Ambassador",width = 13),
                            str_wrap("Podcast: Seeing White",width = 13),
                            str_wrap("E-Documentary: The Student Experience",width = 13)))+  #scale_x_continuous(labels = scales::percent)+
  ...
  #geom_text(aes(label=if_else(prt>.8, as.character(round(prt,2)),"")),hjust=1.5,size=3,fontface="bold",position = position_dodge(0.9))+geom_text(aes(label=if_else(prt<.8, as.character(round(prt,2)),"")),hjust=1.5,
    labs(title="BAM 2021",x="",y="",fill="")+theme_lz()+theme(axis.text.y = element_text(),legend.position = "bottom/top")
```
...
plot.act22<-bam.long%>%#count(act22_value)
  ...
  geom_text(aes(label=percent(prt,digits = 0)),size=3,vjust=0.5,hjust=0.7,fontface="bold",position = position_dodge(0.9))+
  #geom_text(aes(label=if_else(prt>.8, as.character(round(prt,2)),"")),hjust=1.5,size=3,fontface="bold",position = position_dodge(0.9))+geom_text(aes(label=if_else(prt<.8, as.character(round(prt,2)),"")),hjust=1.5,
    labs(title="BAM 2022",x="",y="")+theme_lz()+theme(axis.text.y = element_text(),legend.position = "bottom/top")
```
...
ggplot(aes(y = act_time, x=factor(0))) + geom_boxplot(fill="#69b3e7") +
  ...
  labs(title="",x="Participation Time (Hours)",y="")+
...
ggplot(aes(x=2, y = prt, fill=act_future)) + geom_bar(stat="identity") +
  ...
  #geom_text(aes(label=paste0(prt,"%")), size=3,vjust=-2,hjust=3,fontface="bold",position = position_dodge(1))+
  annotate("text",x = 0.5, y = 0.5, fontface="bold", label = str_wrap( "90% Future Participation", width=10))+
  annotate("text",x = 2, y = 0.75,vjust=0,hjust=-0.5, fontface="bold", label = str_wrap( "10%", width=10))+
  annotate("text",x = 0.5, y = 0.75,vjust=0,hjust=3, fontface="bold", label = str_wrap( "37%", width=10))+
