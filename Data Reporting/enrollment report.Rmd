...
```{r ug and gd}
################ug
ug1<-ug21fa%>%select(Name,`People Code Id`,`gov id`,`FT/PT`,`Transfer YN`,`College Attend`,Ethnicity,Gender,`Term Credits`,`Cum Credits`,Curriculum,`Birth Date`,Degree,Curriculum)%>%
  mutate(sourceLevel="UG",term="21fall")#mutate identification col

ug2<-ug21sum2%>%select(Name,`People Code Id`,`gov id`,`FT/PT`,`Transfer YN`,`College Attend`,Ethnicity,Gender,`Term Credits`,`Cum Credits`,Curriculum,`Birth Date`,Degree,Curriculum)%>%
  mutate(sourceLevel="UG",term="21summer2")

ug3<-ug22sp%>%select(Name,`People Code Id`,`gov id`,`FT/PT`,`Transfer YN`,`College Attend`,Ethnicity,Gender,`Term Credits`,`Cum Credits`,Curriculum,`Birth Date`,Degree,Curriculum)%>%
  mutate(sourceLevel="UG",term="22spring")

ug4<-ug22sum.main.1%>%select(Name,`People Code Id`,`gov id`,`FT/PT`,`Transfer YN`,`College Attend`,Ethnicity,Gender,`Term Credits`,`Cum Credits`,Curriculum,`Birth Date`,Degree,Curriculum)%>%
  mutate(sourceLevel="UG",term="22summer.1main")

ug5<-ug22wi%>%select(Name,`People Code Id`,`gov id`,`FT/PT`,`Transfer YN`,`College Attend`,Ethnicity,Gender,`Term Credits`,`Cum Credits`,Curriculum,`Birth Date`,Degree,Curriculum)%>%
...
################gd
gd1<-g21fa%>%select(Name,`People Code Id`,`gov id`,Gender,`FT/PT`,Ethnicity,`Term Credits`,`Cum Credits`,Degree,`Birth Date`,`New Ret Term YN`,`Transfer YN`,Degree,Curriculum)%>%
  mutate(sourceLevel="GD",term="21fall")#mutate identification col

gd2<-gd21sum2%>%select(Name,`People Code Id`,`gov id`,Gender,`FT/PT`,Ethnicity,`Term Credits`,`Cum Credits`,Degree,`Birth Date`,`New Ret Term YN`,`Transfer YN`,Degree,Curriculum)%>%
  mutate(sourceLevel="GD",term="21summer2")

gd3<-gd22sp%>%select(Name,`People Code Id`,`gov id`,Gender,`FT/PT`,Ethnicity,`Term Credits`,`Cum Credits`,Degree,`Birth Date`,`New Ret Term YN`,`Transfer YN`,Degree,Curriculum)%>%
  mutate(sourceLevel="GD",term="22spring")

gd4<-gd22sum.main.1%>%select(Name,`People Code Id`,`gov id`,Gender,`FT/PT`,Ethnicity,`Term Credits`,`Cum Credits`,Degree,`Birth Date`,`New Ret Term YN`,`Transfer YN`,Degree,Curriculum)%>%
  mutate(sourceLevel="GD",term="22summer.1main")

gd5<-gd22wi%>%select(Name,`People Code Id`,`gov id`,Gender,`FT/PT`,Ethnicity,`Term Credits`,`Cum Credits`,Degree,`Birth Date`,`New Ret Term YN`,`Transfer YN`,Degree,Curriculum)%>%
...
######ug.gd clean class and recode and transfer consistency
...
ug.gd<-ug.gd%>%mutate(degree.c=if_else(Program %in% c("NON","Non Matriculated","Lasell Village"),"nondegree","degree"))

######consistency
#any degree is degree
deg.y<-ug.gd%>%filter(degree.c=="degree")%>%#any ppid with a degree
...
ipeds<-ug.gd%>%select(-`Cum Credits`,-Program,-`gov id`,-`Birth Date`,-term,-`Term Credits`,-sourceLevel,Name)%>%
...
#full time gd men
gd.t1.1<-ipeds%>%filter(gender.assign=="M",#male
               level=="GD",#all gd students
               status=="FT")%>%#full time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(ft.gd.m=n())
#part time gd men
gd.t1.2<-ipeds%>%filter(gender.assign=="M",#male
               level=="GD",#all gd students
               status=="PT")%>%#part time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(pt.gd.m=n())
...
#GD MEN's ethnicity.c by gender
#full time gd men
gd.t2.1<-ipeds%>%filter(gender.assign=="F",#female
               level=="GD",#all gd students
               status=="FT")%>%#full time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(ft.gd.f=n())
#part time gd men
gd.t2.2<-ipeds%>%filter(gender.assign=="F",#female
               level=="GD",#all gd students
               status=="PT")%>%#part time
        group_by(ethnicity.c,.drop = FALSE)%>%#keep zero counts
        summarise(pt.gd.f=n())
...
```{r credit total}
sum(ipeds$UG.tot.credit)
sum(ipeds$GD.tot.credit)
```
...
#enroll.missing<-ipeds%>%filter(ppid%in% type.missing$ppid)%>%select(ppid,level,degree,status)%>%arrange(ppid)
...
#merge
#check: crs.ipeds%>%count(merged.type) #36 na
crs.ipeds<-plyr::join_all(list(crs.ipeds,crstype.found),type = "left")%>%
...
#check: crs.ipeds%>%count(type.c) #4 na

#not found: summer2 students that are not saved in fiscal year 2021-2022 course info
crs.ipeds[is.na(crs.ipeds$type.c),] #P000056440 "P000056986" "P000057211" "P000057212"
#look them up in powercampus and turns out they are all online students
crs.ipeds$type.c[is.na(crs.ipeds$type.c)] <-c("ONLN","ONLN","ONLN","ONLN")
#one of them is due to coursename code error: missing.crsname%>%filter(type=="Unspecified")%>%select(-merged.type,-type) # P000056440
#check: crs.ipeds%>%count(type.c) #zero na
#factorize for order
crs.ipeds<-crs.ipeds%>%mutate(type.c=factor(type.c,levels = c("ONLN","HYB","CAMP")))
#save
write.xlsx(crs.ipeds,file="/Volumes/lasellshare/Faculty_Staff_Shares$/IR/Surveys/IPEDS/2022-2023/Fall Collection/12-month enrollment/crs.ipeds.matched_21-22_rerun.xlsx")
```

```{r distance ed report}
#check: crs.ipeds%>%group_by(ppid)%>%count()%>%ungroup()%>%count(n)#unique rows each ppid
#UGdegree by ONLN vs HYB
ugdeg.distance<-crs.ipeds%>%filter(level=="UG", degree.c=="degree")%>%#UGdegree
        group_by(type.c,.drop = FALSE)%>%#keep zero counts
        summarise(UGdegree=n())
#UGnondegree by ONLN vs HYB
ugnondeg.distance<-crs.ipeds%>%filter(level=="UG", degree.c=="nondegree")%>%#UGnondegree
        group_by(type.c,.drop = FALSE)%>%#keep zero counts
        summarise(UGnondegree=n())
#GDdegree by ONLN vs HYB
gddeg.distance<-crs.ipeds%>%filter(level=="GD")%>%#all GD students
        group_by(type.c,.drop = FALSE)%>%#keep zero counts
        summarise(GD=n())
#merge GD.TABLE 1
