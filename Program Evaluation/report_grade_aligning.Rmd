...
canvrp<-read_excel("/Users/linlizhou/Documents/LASELL/working on/gradalign_working.xlsx")
```
...
writing_bar
```
As shown in Figure \ref{fig.writing}, Writing 101 and 102 has similar exact match, but Writing 101 has 8% significant difference than Writing 102 which is 4%. 
...
To make the grades comparable, we convert grades in Canvas (i.e., numeric grade in scale of 0-100) and Registrar's Office (i.e.,letter grade in scale of A-F) to GPA grades (i.e. numeric grade in scale of 0-4.0). Figure \ref{fig:conversion} shows the distribution of grades in the Canvas and Registrar system before and after conversion.
...
```{r show MDSC/Multidisciplineary Studies, include=TRUE}
